...
**측정의 한계 (2026-09-02)**: clean commit `ed846d920`에서 연속 도착 기준선을
6회 측정했다 — 50.08 / 50.40 / 50.45 / 52.82 / 53.41 / 54.96 tok/s, 폭
6.07~6.60행. **산포가 약 10%다.** 앞선 여러 바이너리에서 얻은 기준선 표본
(50.32~53.18)과 거의 완전히 겹치므로, 이 산포에서 **10% 미만의 변화는 어느
쪽으로도 판정할 수 없다** — 리팩터링의 성능 영향이 있는지 없는지를 이 표본
수로는 말할 수 없다는 뜻이다.

이것은 작업의 공백이 아니라 **측정의 성질**이다. 판정하려면 표본을 크게 늘리거나
산포가 낮은 지표가 필요하다. 파편화가 실행마다 달라지는 것이 산포의 원인으로
보이므로, 노드가 자기 배치 폭을 보고하게 되면(P1a) 폭을 조건으로 나눈 비교가
가능해진다. 그때까지 이 문서는 **비악화를 주장하지 않는다**.

같은 커밋의 임계값 24 측정은 58.74 / 58.93으로 훨씬 촘촘하다. 병합이 파편화를
제거하므로 산포도 함께 줄어드는 것으로 보이며, 이는 임계값의 두 번째 효과다 —
처리량뿐 아니라 **예측 가능성**을 바꾼다.

...
| 웨이브 | 기본 | 1 | 34.22 | 303.5 | 110.6 | 2 |
| 웨이브 | 임계값 40 | 1 | 34.25 | 305.3 | 110.1 | 2 |
...
넓으므로 **2표본으로는 이득도 손해도 주장할 수 없다**. 말할 수 있는 것은
8월 31일의 손해가 이 하드웨어에서 재현되지 않는다는 것뿐이다. 웨이브에서는
임계값이 아무것도 바꾸지 않는다 — 20건이 한꺼번에 도착해 배치가 이미 차 있다.
...
아니라 비대칭 스테이지 배치의 성질이었다.** 채움 규칙 "고정비 대비 한계 이득이
양수인 동안 채운다"는 유지되고, 그 한계 이득의 부호는 **구성마다 재측정으로만
정해진다**. 임계값은 계약으로 남는다.

**기본값이 여전히 꺼짐인 이유는 처리량이 아니라 지연이다.** 임계값 24는 같은
처리량을 스텝당 116 ms 대신 264 ms로 낸다. 대화형 부하에서 스텝 지연이 두 배가
되는 대가로 얻는 것이 동률이면 켤 이유가 없다. cut-set이 훨씬 넓은 모델,
스냅샷 정합 펜스(불변식 11), 등폭 강제 계열에서는 부호가 바뀔 수 있고, 그때
재측정으로 결정한다.

...
| 웨이브 | 임계값 40 | 1 | 34.19 | 300 | **111.8** | 0 |
...
행이 2.5배 늘 때 스텝 시간이 2.1배 늘었다 — **선형 미만**이므로 넓힐수록 이득이다.

**따라서 임계값의 부호는 정책이 아니라 파편화의 유무가 정한다.** 배치가 이미
합쳐져 있으면 임계값은 아무것도 하지 않고, 파편화가 일어나 있으면 그것을
되돌린다. 그리고 파편화 자체가 타이밍에 민감하다 — 같은 시나리오·같은 pin이
스테이지 바이너리가 바뀌자 병합(1,190 배치)에서 파편화(3,000 배치)로 넘어갔다.

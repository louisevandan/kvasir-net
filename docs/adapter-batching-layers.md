...
**2026-09-01 재측정 (RTX 3090 x2, 대칭 배치): 그 손해는 재현되지 않았다.**

재측정한 이유는 둘이다. 하드웨어가 바뀌었고, 그때의 드라이브 리더가
취소-비안전이라 도착 웨이브마다 이벤트를 조용히 삼킬 수 있었다(교정 완료,
복원 계획 O13). 배치 텔레메트리를 잃으면 배치가 많은 쪽이 더 많이 잃으므로
기본값 쪽이 불리해질 뿐 위 관측의 부호를 만들지는 못한다 — 즉 리더 결함은
이 표를 설명하지 못하고, 남는 설명은 스테이지 배치의 대칭성이다.

| 시나리오 | 정책 | 표본 | rows/batch | ms/batch | 생성 TPS | 혼합 배치 |
| --- | --- | --- | --- | --- | --- | --- |
| 연속 도착 | 기본 | 3 | 6.4~7.3 | 113~116 | 52.9 / 54.0 / 59.0 | 5~6 |
| 연속 도착 | 임계값 24 | 2 | 16.8 | 264~265 | 59.7 / 60.0 | 19 |
| 웨이브 | 기본 | 4 | 13.6~18.6 | 140~142 | 91.1 / 93.8 / 95.0 / 95.3 | 0 |
| 웨이브 | 임계값 40 | 1 | 34.19 | 300 | **111.8** | 2 |

두 표의 절대값은 비교 대상이 아니다 — 하드웨어도 하네스 판본도 다르다.
비교되는 것은 각 표 **안에서의** A/B 방향뿐이다.

연속 도착에서 행이 2.3배 늘 때 스텝 시간도 2.3배 늘었다 — **선형**이며,
따라서 처리량은 동률이다. 기본값의 표본 폭(52.9~59.0)이 두 정책의 차이만큼
넓으므로 **2표본으로는 이득도 손해도 주장할 수 없다**.

웨이브는 다르다. 여기서는 파편화가 실제로 일어나 3,000개 배치가 문서가
예측한 그대로 20행 1000회 + 10행 2000회로 라운드로빈했고, 임계값 40이 그
무리를 합쳐 폭 13.57 → 34.19, 처리량 94 → **111.8 tok/s(+18%)**로 회복시켰다.
행당 wall time은 8.9 ms에서 8.4 ms로 내려갔다.

**다만 이것을 compute 비용의 증명으로 읽어서는 안 된다.** 이 표의 `ms/batch`는
스테이지 실행 시간이 아니라 `wall_s * 1000 / physical_batches`이고, 웨이브 대기·
큐잉·네트워크·파이프라인 유휴가 전부 들어 있다. `rows/batch`와 분모가 같으므로
"행 2.5배에 스텝 2.1배"는 처리량 비를 다시 쓴 것에 가깝다. 실제 compute 비용의
선형성 여부는 스테이지가 배치 submit/start/end를 직접 보고해야 말할 수 있고,
그 계측은 아직 없다.

**현재 데이터가 지지하는 진술은 여기까지다**: 식별된 이 서버 빌드와 3090×2의
service 부하에서, 임계값 40 실행 1건이 기본값 4건 평균 약 94.7에서 111.8 tok/s로
올랐고 물리 배치가 3,000에서 1,191로 줄었다. "부호는 파편화가 정한다"는 이
관측과 정합하는 **가설**이지 확정된 계약이 아니다 — TTFT·ITL·SLA, 스테이지
비대칭, cut-set 크기, 파이프라인 깊이(현재도 1), backend가 모두 부호를 바꿀 수
있고, threshold-40 표본은 아직 1건이다.


**계약 정정 (2회차)**: 앞서 이 문서가 "고정비가 지배하므로 빈 행 슬롯은 순수한
낭비"라고 적었던 것은 서로 다른 실행을 비교한 결과였고, 동일 조건 A/B가 그것을
뒤집었다. 이번에는 그 정정 자체를 좁힌다 — **"폭을 넓히면 손해"는 일반 성질이
아니었다.** 채움 규칙 "고정비 대비 한계 이득이 양수인 동안 채운다"는 유지되고,
그 한계 이득의 부호는 **배치가 이미 합쳐져 있는지**가 정한다.

**기본값이 꺼짐인 이유는 지연이고, 켜야 할 조건은 파편화다.** 임계값은 파편화가
없을 때 스텝 지연만 두 배로 만들고(116 → 264 ms) 아무것도 얻지 못한다. 파편화가
있을 때는 +18%를 준다. 즉 이것은 상시 정책이 아니라 **관측에 반응하는 정책**이며,
노드가 자기 배치 폭의 파편화를 텔레메트리로 보고해야 성립한다(P1a).

**출처 없는 수치의 지위**: 8월 31일 표의 실행들, 그리고 9월 1일 오전의 웨이브
실행 두 건(110.6·110.1 tok/s, 폭 34.2)은 provenance를 남기지 않는 스테이지
바이너리로 측정됐다. 그 바이너리는 지금 없고 어떤 소스 상태로 빌드된 것인지
확정할 수 없다 — 저장소 pin은 같다고 말하지만 그것은 저장소의 주장일 뿐
바이너리의 증언이 아니다. 같은 시나리오가 바이너리 교체만으로 병합(1,190 배치)에서
파편화(3,000 배치)로 넘어갔으므로 차이는 실재한다. 그 수치들은 기준선이 아니라
**식별 불가능한 빌드의 관측**으로 읽어야 하며, 이것이 HELLO에 `patch_set`을 넣고
실행 산출물이 그것을 기록하게 만든 이유다.

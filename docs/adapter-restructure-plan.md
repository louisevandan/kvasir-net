...
빌드 아키텍처 함정도 기록해 둔다. 스크립트 기본값이 `75;89`인데 이 배치의
3090은 sm_86이라 네이티브 코드가 없어 처리량이 19.9 → 6.15 tok/s로 떨어졌다.
정답은 실측한 compute capability(`86;89`)다. 수용 판정이 처리량이 아니라
**답의 의미**였기에 이 함정은 시험을 통과시키면서도 드러났다 — 두 판정을
분리해 둔 것이 값을 한 지점이다.

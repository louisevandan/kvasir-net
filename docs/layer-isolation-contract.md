...
2026-09-07 보강. 최초 코드 관찰 기준 `a9e1967fc`; 각 후속 관찰은 그 당시 소스에 한정한다.
현재 체크포인트·검증 상태·미검증 후보와 중단 조건은 [로드맵 §0](distributed-batching-roadmap.md#current-status)을 따른다.
...
### 2.1 HF 구상 어댑터 적용 (2026-09-14 이관)

`layers/adapters/hf/`는 P4가 소유하는 구상 어댑터이며 별도 workspace가 아니다.
entrypoint는 `hf-transformers`만 등록한다. 모델별 의미와 Python worker는 HF 내부에 둔다.
...
- 격리·소유권·의미 호환 시험은 [HF 검증](distributed-batching-verification.md#hf-integration-contract)을 유지한다.

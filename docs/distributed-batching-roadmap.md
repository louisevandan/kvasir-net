...
최신 현황 정리: 2026-09-07. 최초 감사 기준: `a9e1967fc59dffa6c2e458f1b91f916b1df826c1`.
...
## 0. 2026-09-07 중단 시점 — 검증된 진전과 남은 일

사용자가 구현 전진 대신 **지금까지 확실히 전진한 것과 앞으로 할 일의 문서화**를 요청했다.
이에 구현·컴파일·시험·변이·실기 실행을 중단했다. 아래 계획은 재개 시 사용할 인수인계이며
자동 실행 지시가 아니다. 문서 작성 전 HEAD는 `6fe10eb104b783f6ea5cacbc2e549a0a7e504677`이다.
...
resident 32, ctx 2560/seq)의 `report.json`·`artifact.json`에서 옮겼다.

#### P-0. 현재 처리량이 무엇으로 결정되는지 (측정됨)

관측된 값으로 처리량이 정확히 재구성된다. 추정이 아니라 항등식이다.

| 항목 | 값 |
| --- | ---: |
| 물리 batch | 2,459회 / 325.87 s = 7.55 batch/s |
| batch 주기 | 132.5 ms |
| 그중 stage 계산(node 0 평균) | 82.6 ms |
| 그중 유휴(node 0 평균) | 48.6 ms |
| decode batch 폭 평균 | 15.80행 (p50 10, 최대 30, UBATCH 512) |
| prefill batch 폭 평균 | 374.37행 (49회) |
| prefill과 decode가 한 물리 batch에 함께 든 횟수 | **0 / 2,459** |
| 두 stage가 동시에 계산한 시간 비율 | 31.6% |
| gate 거절 | 0회 |
| 발행 시점의 ready 행 | 평균 86.4행 |

`7.55 × 15.80 = 119 row/s`이고 보고된 생성 TPS는 117.07이다. `82.6 + 48.6 = 131.2 ms`는 주기
132.5 ms와 같다. 즉 **처리량 = (1 / (stage + 유휴)) × decode 폭**이며, 세 항 모두 측정돼 있다.

같은 실행의 오프로딩 대조(`20260907T102817Z-8438348c`, 전문가 CPU)는 stage p50 233 ms,
유휴 116.7 ms, 주기 354.3 ms로 43.19 TPS다. 유휴 비율은 33%로 비슷하지만 stage 자체가 2.8배 길다.
**배치 전략이 다르면 병목도 다르다**는 뜻이므로 두 경로의 최적화를 같은 항목으로 묶지 않는다.

#### P-1. 이미 아는 것과 그래서 하지 말아야 할 것

- `worker/drive.rs`의 발행 폭 주석은 한 시나리오 18회 실행에서 **처리량이 “두 stage 이상이 계산
  중인 시간 비율”과 r=0.891로 함께 움직였고, batch 폭(r=-0.357)과 UBATCH 채움과는 오히려 약하게
  반대로 움직였다**고 기록한다. 같은 주석이 첫 층 이전의 batch 고정비를 약 55 ms로 적는다.
- 2026-08-31 관측도 같은 방향이다. `min_batch_rows`로 폭을 2.84→10.54행으로 넓히자 생성 TPS는
  108.9→96.9로 내려갔다.
- 따라서 **UBATCH 채움률을 목표로 삼지 않는다.** 4.5%라는 숫자 자체는 결함이 아니다. 목표는
  주기에서 유휴를 걷어내는 것과, 폭을 늘리더라도 stage 시간을 늘리지 않는 방법을 찾는 것이다.

#### P-2. 먼저 복구해야 하는 측정 신뢰 (§0.0의 1~3과 같은 항목)

성능 판정을 시작하기 전에 끝낸다. 이유는 세 가지 모두 **성능 실험의 결과를 못 읽게 만들기** 때문이다.

1. HEAD lib test 컴파일 복구. 회귀 없이 스케줄러를 만지지 않는다.
2. `run/mod.rs`가 UNLOAD 전에 `InferenceResult::error`와 부분 결과를 보존하도록 고치고,
   `run.mjs::stopChild`의 신호 종료 오판을 고친다. 지금은 실패 원인이 UNLOAD busy로 덮인다.
3. `pressure`(resident 256, 512요청) 재판정. **이 시나리오가 P-4의 주력 측정 도구다.**
   resident를 올린 상태의 처리량을 재려면 이 경로가 살아 있어야 한다.

#### P-3. 유휴 48.6 ms의 귀속 (계측만, 정책 변경 없음)

gate 거절이 0이고 ready 행이 평균 86개 있는데도 첫 노드가 주기의 37%를 쉬었다. 원인을 **지금은
모른다.** 다음을 한 실행 안에서 같은 시각축으로 기록해 귀속시킨다.

- 발행이 막힌 사유를 행 단위로 분류한다: 앞 batch의 반환 대기, KV/credit 부족, shape 제약,
  같은 요청의 다음 fragment 금지, 원격 수용 대기, 재시도 간격. `ready_rows`가 “남은 행”인지
  “발행 시점의 전체 준비 행”인지도 이름과 함께 확정한다(현재 어댑터는 후자, 하네스 필드명은 전자다).
- batch 고정비 55 ms의 내역을 나눈다: frame decode, `llama_decode` 진입까지, cut-set 복사,
  프로세스 홉, sampler. 어느 항이 지배적인지 모르면 무엇을 줄일지 정할 수 없다.
- 두 stage가 동시에 계산한 31.6%가 왜 더 높지 않은지 본다. launcher는 `MAX_OPEN_BATCHES=0`
  (무제한)이었으므로 이 knob이 원인이 아니다. 직렬화의 실제 지점을 찾는다.

이 단계의 산출물은 **“무엇을 기다렸는가”의 분포**다. 여기서 나온 1순위 원인 없이 P-4로 넘어가지 않는다.

#### P-4. 귀속 결과가 정당화할 때만 하는 변경

각 항목에 “이 측정이 나오면 한다”를 함께 적는다. 순서는 기대 효과가 아니라 P-3의 결과가 정한다.

| 후보 | 근거가 될 측정 | 기대 |
| --- | --- | --- |
| resident 상향(32 → 64/128/256) | decode 폭이 `resident × stage/주기`로 결정됨을 P-3이 확인. 35B는 seq 32에서 stage당 KV 425 MiB뿐이라 예산 여유가 큼 | 폭이 선형에 가깝게 증가. 폭의 음의 상관이 약하므로 주기를 크게 늘리지 않는 한 이득 |
| 앞 batch 반환 전 다음 batch 발행 | P-3이 유휴의 1순위를 “반환 대기”로 지목 | 유휴 제거 시 7.55 → 12.1 batch/s. 폭이 유지되면 117 → 약 191 TPS |
| batch 고정비 절감 | P-3의 55 ms 내역에서 지배 항이 나옴 | 주기 직접 단축. 폭과 무관하게 이득 |
| prefill·decode 혼합 복원 | 35B에서 0/2,459인 이유가 정책인지 도착 패턴인지 P-3이 판정. 2B 실행에서는 71/3,020으로 발생함 | prefill 대기 중 decode 정지 제거 |
| 1 ms 재시도를 capacity wake로 교체(§0.5 2번과 공유) | 유휴 분포에 재시도 간격이 보이면 | 짧은 대기의 하한 제거 |
| 오프로딩 경로의 stage 233 ms 분해 | 전문가 CPU 계산과 host↔device 전송의 비중 | 이 경로는 위 항목과 병목이 다름. 별도 판정 |

resident 상향은 §0.5 3번(수용·비행 budget)과 같은 코드를 건드린다. 성능 목적으로 먼저 올리지 말고
그 항목의 예약·거절·멱등 정산을 먼저 세운 뒤 올린다. 슬롯 수를 KV 예산으로 부르지 않는다.

#### P-5. 판정 규율

- 비교는 검증 규약 H5를 따른다. 같은 모델·요청·토폴로지·KV 용량·resident·backend에서 정책만 바꾸고,
  paired 8회 이상, holdout 4쌍, 중앙 개선 ≥5%, TTFT p95 ≤1.10배, ITL p95 ≤1.05배를 지킨다.
- **GPU 사용률만으로 승격하지 않는다.** 사용률이 올라도 유효 생성 TPS가 내려가면 기각한다.
  반대로 TPS만 오르고 사용률이 내려간 결과를 “GPU 활용까지 개선”이라 쓰지 않는다.
- 사용률을 인용할 때는 분석창을 밝힌다. 적재·정리를 포함한 전체 캡처와 stage 실행창은 다르다
  (35B는 각각 16.1/16.7%와 29.3/30.7%였다).
- 처리량 수치에는 계산식과 품질 판정 여부를 함께 적는다. 현재 `generation_tps`는 품질 판정 전 값이다.

...
- **사용률을 정하는 것은 모델 크기다.** 같은 워크로드에서 2B는 GPU 21.4~32.9 %, 35B는 36.8~50.7 %,
  전력은 99~160 W 대 175~213 W다. 2B는 한 행이 카드를 채우지 못해 행 수를 늘려도 따라오지 않는다.
- **한 카드 한 stage.** resident를 고정하고 분할만 바꾸면 4 stage가 33 % 느리다(275.35 대 183.00).
  10층 stage 97 ms·20층 stage 117 ms에서 **stage당 고정비 약 77 ms**가 나온다.
  4 stage가 resident 256에서 좋아 보였던 것은 분할이 아니라 resident 때문이다.
- **UBATCH 채움률은 목표 지표가 아니다.** `min-batch-rows 64`는 채움률을 9.09 → 25.14 %로 올리고
  TPS를 19 %, GPU를 40 % 떨어뜨렸다(겹침 73 → 3 %, 깊이 3.29 → 1.03). 포화는 **카드가 쉬지 않는
  정도**로 읽는다.
- **폭을 정하는 것은 resident가 아니라 도착 버스트다.** resident 96 → 160에서도 decode 폭은 32와 64
  둘뿐이었다. 도착률을 고정하고 버스트만 키우면 폭·TPS가 함께 오르고 64건에서 포화한다
  (4건/125 ms 223.10 → 32건/1 s 289.79 → **64건/2 s 313.80** → 128건/4 s 302.49).
- **적재 판정이 최적 구성을 막는다.** `stage_memory_plan.cpp`의 `required() <= free`에서 `free`는
  recurrent 버퍼를 이미 뺀 값인데 `required`가 그 버퍼를 다시 센다(5회 계획에서 `free + RS = 22.76 GiB`).
  실효 조건이 `model + compute + 2 × context`가 되어 2 stage의 resident 상한이 절반으로 깎인다.
  **staged 서버이므로 이 세션에서 고치지 않았다.**

**측정된 최선: 35B, 한 카드 한 stage, resident 160, 64건 2초 버스트 — 313.80 TPS,
토큰 간격 478.7 ms, GPU 41.8 / 40.8 %.** 다음 순서는 위 증거의 마지막 절이 소유한다.

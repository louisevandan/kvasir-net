...
6. **실패한 실행의 부분 결과 보존.** **2026-09-09 완료.**
   - 09-09 포화 실험의 4-stage 실패 4회가 `artifact.json`을 남기지 못했다. 출력 583건을 받은 뒤
     10054로 끊겼는데, `inference::drive`가 루프 안의 실패를 `?`로 내보내 모아 둔 요청·출력·완료·
     관측이 통째로 사라졌고 `assemble`에 닿지 못했다. **최초 원인을 관측 증거 누락·스트림 중단·
     10054로 구분할 수 없었다.**
   ...
   - 실제 소비 경로에 연결 절단·deadline 만료·잘못된 후속 이벤트를 각각 주입하는 시험 4개와,
     그 위에 UNLOAD 거부를 얹는 시험. 워크스페이스 **1,371 passed / 0 failed / 7 ignored**.
     변이(오류 재전파)는 분리된 worktree에서 5개 실패로 확인했다.
   - **09-09의 10054 자체는 최초 원인 미확정이다.** 재시도 성공으로 닫지 않는다.
...
`length` 종료인 **고정 길이 부하시험**이고, 기본 r160의 수용 대기 중앙값은 93.9 초다. 정상 서비스
승인이 아니다. 다음 순서는 위 증거의 마지막 절이 소유한다.

    ...
    W: AsyncWrite + Unpin,
{
    ...
    pub fn acknowledged(reader: R, writer: W, sender_id: String, generation: u64) -> io::Result<Self> {
        if sender_id.is_empty() || generation == 0 {
            return Err(io::Error::other("acknowledged EventWire requires identity and generation"));
        }
        Ok(Self { reader, writer, buffer: Vec::new(), cursor: 0, finishing: false,
            hop: Some(HopState { sender_id, generation, next_attempt: 1,
                max_outstanding: 256, handshaken: false, sent: HashMap::new(),
                received: HashMap::new(), pending: VecDeque::new(), rejected_local: Vec::new(),
                confirmed: Vec::new(), peer_sender_id: None, peer_generation: None }) })
    }

    pub fn acknowledged_mode(&self) -> bool { self.hop.is_some() }

    pub fn take_confirmed(&mut self) -> Vec<String> {
        self.hop.as_mut().map(|state| std::mem::take(&mut state.confirmed)).unwrap_or_default()
    }
    ...
    pub async fn send(&mut self, event: Event) -> io::Result<()> {
        ...
        if self.hop.is_some() {
            ...
            let bytes = match encode(&event) {
                Ok(value) => value,
                Err(error) => {
                    self.hop.as_mut().expect("hop mode").rejected_local.push(event);
                    return Err(io::Error::other(error));
                }
            };
            let digest = hop::event_digest(&bytes);
            let attempt = self.hop.as_ref().expect("hop mode").next_attempt;
            let frame = hop::encode(&HopFrame::Data { attempt, digest, event: bytes })
                .map_err(io::Error::other)?;
            let state = self.hop.as_mut().expect("hop mode");
            state.next_attempt = state.next_attempt.checked_add(1)
    ...
    pub async fn finish(&mut self, deadline: Instant) -> io::Result<()> {
        ...
        if self.hop.is_some() {
            ...
            while !self.hop.as_ref().expect("hop mode").sent.is_empty()
                ...
                tokio::time::timeout(remaining, self.progress_hop()).await
                    .map_err(|_| io::Error::new(io::ErrorKind::TimedOut, "hop receipt timeout before connection finish"))??;
            }
            if !self.hop.as_ref().expect("hop mode").pending.is_empty() {
                return Err(io::Error::other("unconsumed events before connection finish"));
            }
        }
    ...
    async fn ensure_hop(&mut self) -> io::Result<()> {
        if self.hop.as_ref().is_none_or(|state| state.handshaken) { return Ok(()); }
        let state = self.hop.as_ref().expect("hop mode");
        let hello = hop::encode(&HopFrame::Hello { sender_id: state.sender_id.clone(),
            connection_generation: state.generation, max_outstanding: state.max_outstanding as u32,
            max_receipt_bytes: 64 * 1024 * 1024 }).map_err(io::Error::other)?;
        self.write_body(&hello).await?;
        let body = self.read_body().await?;
        let frame = hop::decode(&body).map_err(io::Error::other)?
            .ok_or_else(|| io::Error::other("peer does not support acknowledged hop transport"))?;
        match frame {
            HopFrame::HelloAck { accepted_connection_generation, sender_id,
                connection_generation, max_outstanding, .. }
                if accepted_connection_generation == self.hop.as_ref().expect("hop mode").generation => {
                let state = self.hop.as_mut().expect("hop mode");
    ...
    async fn progress_hop(&mut self) -> io::Result<()> {
        ...
        match frame {
            HopFrame::Receipt { attempt, digest, status, detail } => {
                let state = self.hop.as_mut().expect("hop mode");
                let sent = state.sent.get(&attempt)
                    .ok_or_else(|| io::Error::other("receipt has no outstanding attempt"))?;
                if sent.digest != digest { return Err(io::Error::other("receipt digest conflict")); }
                if status != ReceiptStatus::AcceptedExact {
                    return Err(io::Error::other(format!("hop receipt refused Event: {status:?}: {detail}")));
                }
                let confirmed = state.sent.remove(&attempt).expect("checked outstanding receipt");
                state.confirmed.push(confirmed.event.envelope.event_id);
                let state = self.hop.as_ref().expect("hop mode");
                let ack = hop::encode(&HopFrame::ReceiptAck { sender_id: state.sender_id.clone(),
                    connection_generation: state.generation, attempt, digest }).map_err(io::Error::other)?;
                self.write_body(&ack).await
            }
            HopFrame::Data { attempt, digest, event: bytes } => {
                let existing = self.hop.as_ref().expect("hop mode").received.get(&attempt).copied();
                if let Some(existing) = existing {
                    let status = if existing == digest { ReceiptStatus::AcceptedExact }
                        else { ReceiptStatus::Conflict };
                    let receipt = hop::encode(&HopFrame::Receipt { attempt, digest, status,
                        detail: if status == ReceiptStatus::Conflict { "attempt digest differs".into() }
                            else { String::new() } }).map_err(io::Error::other)?;
                ...
                let receipt = hop::encode(&HopFrame::Receipt { attempt, digest,
                    status: ReceiptStatus::AcceptedExact, detail: String::new() }).map_err(io::Error::other)?;
            ...
            HopFrame::ReceiptAck { sender_id, connection_generation, attempt, digest } => {
                ...
                if state.peer_sender_id.as_deref() == Some(sender_id.as_str())
                    && state.peer_generation == Some(connection_generation)
                    && state.received.get(&attempt).is_some_and(|value| *value == digest) {
                    state.received.remove(&attempt);
                }
                Ok(())
            }
            HopFrame::Query { attempt, digest, .. } => {
                let known = self.hop.as_ref().expect("hop mode").received.get(&attempt).copied();
                ...
                let result = hop::encode(&HopFrame::QueryResult { attempt, digest, status,
                    detail: if status == ReceiptStatus::Unknown { "receipt is not pinned".into() }
                        else if status == ReceiptStatus::Conflict { "attempt digest differs".into() }
                        else { String::new() } }).map_err(io::Error::other)?;
    ...
    async fn write_body(&mut self, bytes: &[u8]) -> io::Result<()> {
        let size = u32::try_from(bytes.len()).map_err(|_| io::Error::other("event too large"))?;
        if bytes.len() > MAX_FRAME { return Err(io::Error::other("event too large")); }
        self.writer.write_u32_le(size).await?;
        self.writer.write_all(bytes).await?;
        self.writer.flush().await
    }
    ...
    async fn read_body(&mut self) -> io::Result<Vec<u8>> {
        loop {
            if let Some(body) = self.take_body()? { return Ok(body); }
            let read = self.reader.read_buf(&mut self.buffer).await?;
            if read == 0 { return Err(io::Error::new(io::ErrorKind::UnexpectedEof, "event stream ended mid-frame")); }
        }
    }
    ...
    fn take_body(&mut self) -> io::Result<Option<Vec<u8>>> {
        ...
        if available < HEADER { return Ok(None); }
        let size = u32::from_le_bytes(self.buffer[self.cursor..self.cursor + HEADER]
            .try_into().expect("four-byte frame prefix")) as usize;
        if size == 0 || size > MAX_FRAME { return Err(io::Error::other("invalid event size")); }
        if available < HEADER + size { return Ok(None); }
    ...
    fn take_frame(&mut self) -> io::Result<Option<Event>> {
        self.take_body()?.map(|body| decode(&body).map_err(io::Error::other)).transpose()
    }
}

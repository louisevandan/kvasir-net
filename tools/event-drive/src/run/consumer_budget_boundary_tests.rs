...
fn config(case: Case) -> RunConfig {
    ...
    RunConfig {
        ...
        prompt: String::new(),
        prompts: if matches!(case, Case::GroundedPower | Case::GroundedWrongRevision) {
            vec![format!(
                "<|im_start|>user\nCase 5: 1 archived records.\n[R00002] Station 7; revision 6. Measured RMS current: 28 A. Isolated conductor resistance: 52 milliohms. Operating duration: 10 hours. Inlet pressure: 121 kPa. Pressure alarm threshold: 120 kPa; equality is not an exceedance. Temperature was not measured.\n\nConnect the source facts for R00002, in that order. {}<|im_end|>",
                source_grounded::TASK_SUFFIX
            )]
        } else if count == 1 {
        ...
        response_processors: if matches!(case, Case::GroundedPower | Case::GroundedWrongRevision) {
            vec![Some(ResponseProcessor::EngineeringPowerV1)]
        } else {
            Vec::new()
        },
        timeout_ms: 2_000,
...
#[tokio::test]
async fn actual_outer_consumer_computes_from_source_and_preserves_model_text() {
    let result = approved(Case::GroundedPower).await;
    ...
    assert_eq!(value["rows"][0]["power_mW"], 40768);
    assert_eq!(value["rows"][0]["energy_mWh"], 407680);
    assert_eq!(value["rows"][0]["pressure_alarm"], true);
}

#[tokio::test]
async fn actual_outer_consumer_rejects_wrong_source_identity_after_terminal_and_release() {
    let result = exercise(Case::GroundedWrongRevision).await.unwrap();
    let request = &result.run.requests[0];

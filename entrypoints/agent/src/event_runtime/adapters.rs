...
const FACTORIES: &[(&str, Factory)] = &[
    ("llamacpp", |endpoint, input, output, retained, bytes, probe| {
    ...
    ("hf-transformers", |endpoint, input, output, retained, bytes, _probe| {
...
mod tests {
    ...
    fn probe() -> RuntimeResourceProbe {
        RuntimeResourceProbe::new(|| {
            ...
            Ok(RuntimeResourceSnapshot { edge: store, receipt: store })
        })
    }
    #[tokio::test]
    async fn advertised_factories_construct_the_real_retained_adapters() {
        ...
        assert!(create("unknown",Endpoint::node("tcp://127.0.0.1:41990".parse().unwrap(),"invalid",1),1,1,2,4096,probe()).is_err());
        #[cfg(not(feature="hf-transformers"))]
        assert!(create("hf-transformers",Endpoint::node("tcp://127.0.0.1:41990".parse().unwrap(),"disabled",1),1,1,2,4096,probe()).is_err());
    }
}

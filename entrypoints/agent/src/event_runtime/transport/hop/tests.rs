...
fn reservation_commit_query_and_ack_have_one_owner() {
    ...
    assert_eq!(
        store.reserve(key(1), digest, "event-1"),
    ...
    let accepted = store
        .commit(&key(1), digest, ReceiptStatus::AcceptedExact, "")
        .unwrap();
    assert_eq!(accepted.status, ReceiptStatus::AcceptedExact);
    assert_eq!(
        store.reserve(key(1), digest, "event-1"),
        Ok(Reservation::Existing(accepted.clone()))
    );
...
fn count_and_byte_boundaries_refuse_without_mutation() {
    ...
    assert_eq!(
        bytes_short.reserve(key(1), digest, "event-1"),
        Err(ReserveError::TooLarge)
    );
    assert_eq!(bytes_short.snapshot().records, 0);
    let mut count_full = ReceiptStore::new(1, required * 2);
    assert_eq!(
        count_full.reserve(key(1), digest, "event-1"),
        Ok(Reservation::Reserved)
    );
    assert_eq!(
        count_full.reserve(key(2), digest, "event-2"),
...
fn digest_conflict_and_oversized_detail_preserve_the_original_record() {
    ...
    assert_eq!(
        store.reserve(key(1), event_digest(b"conflict"), "event-1"),

...
async fn acknowledged_tcp_ingress_commits_once_and_ack_releases_pinned_receipt() {
    ...
    let shared = Shared::with_identity("agent-receiver".into(), RuntimeLimits {
        queue: 4, retained: 8, bytes: 1024 * 1024, connections: 2,
        hop_receipts: 2, hop_receipt_bytes: 1024 * 1024, hop_outstanding: 2 });
    ...
    let mut client = TcpStream::connect((own.host.as_str(), own.port)).await.unwrap();
    ...
    let hello = hop::encode(&HopFrame::Hello { sender_id: "outer-a".into(),
        connection_generation: 7, max_outstanding: 2, max_receipt_bytes: 1024 * 1024 }).unwrap();
    write_bytes_frame(&mut client, &hello).await.unwrap();
    assert!(matches!(hop::decode(&read_body(&mut client).await.unwrap().unwrap()).unwrap(),
        Some(HopFrame::HelloAck { accepted_connection_generation: 7, .. })));
    let event = super::super::tests::event(&own, Endpoint::agent(own.clone()), 1,
        "application/octet-stream", b"opaque");
    let bytes = encode(&event).unwrap();
    let digest = hop::event_digest(&bytes);
    write_bytes_frame(&mut client, &hop::encode(&HopFrame::Data {
        attempt: 1, digest, event: bytes }).unwrap()).await.unwrap();
    assert_eq!(next(&incoming).await.unwrap().event(), &event);
    let receipt = hop::decode(&read_body(&mut client).await.unwrap().unwrap()).unwrap().unwrap();
    assert!(matches!(receipt, HopFrame::Receipt { attempt: 1,
        status: ReceiptStatus::AcceptedExact, .. }));
    assert_eq!(shared.receipts.lock().unwrap().snapshot().accepted, 1);
    write_bytes_frame(&mut client, &hop::encode(&HopFrame::ReceiptAck {
        sender_id: "outer-a".into(), connection_generation: 7,
        attempt: 1, digest }).unwrap()).await.unwrap();
    tokio::time::timeout(std::time::Duration::from_secs(2), async {
        while shared.receipts.lock().unwrap().snapshot().records != 0 { tokio::task::yield_now().await; }
    }).await.unwrap();
...
async fn full_receipt_store_rejects_before_broker_commit() {
    use p4_protocol::event::hop::{self, HopFrame, ReceiptStatus};
    let limits = RuntimeLimits { queue: 2, retained: 4, bytes: 1024 * 1024, connections: 2,
        hop_receipts: 1, hop_receipt_bytes: 1024 * 1024, hop_outstanding: 1 };
    let shared = Shared::with_identity("receiver".into(), limits);
    let occupied = super::hop::ReceiptKey { sender_id: "other".into(), connection_generation: 1, attempt: 1 };
    let occupied_digest = hop::event_digest(b"occupied");
    shared.receipts.lock().unwrap().reserve(occupied, occupied_digest, "occupied").unwrap();
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let own = Address::tcp("127.0.0.1", listener.local_addr().unwrap().port());
    let (agent, incoming) = shared.limits.mailbox();
    let (outer, _outer) = shared.limits.mailbox();
    ...
    let (accepted, _) = listener.accept().await.unwrap();
    let slot = shared.slots.clone().acquire_owned().await.unwrap();
    let task = tokio::spawn(serve(202, accepted, broker.clone(), shared.clone(), Arc::new(slot)));
    write_bytes_frame(&mut client, &hop::encode(&HopFrame::Hello { sender_id: "outer".into(),
        connection_generation: 2, max_outstanding: 1, max_receipt_bytes: 1024 }).unwrap()).await.unwrap();
    let _ = read_body(&mut client).await.unwrap();
    let event = super::super::tests::event(&own, Endpoint::agent(own.clone()), 1,
        "application/octet-stream", b"must-not-commit");
    let bytes = encode(&event).unwrap();
    let digest = hop::event_digest(&bytes);
    write_bytes_frame(&mut client, &hop::encode(&HopFrame::Data { attempt: 1, digest,
        event: bytes }).unwrap()).await.unwrap();
    let receipt = hop::decode(&read_body(&mut client).await.unwrap().unwrap()).unwrap().unwrap();
    assert!(matches!(receipt, HopFrame::Receipt { status: ReceiptStatus::Rejected, .. }));
...
async fn explicit_reconcile_queries_exact_receipt_before_resuming_original_queue() {
    ...
    let limits = RuntimeLimits { queue: 4, retained: 8, bytes: 1024 * 1024, connections: 8,
        hop_receipts: 8, hop_receipt_bytes: 1024 * 1024, hop_outstanding: 2 };
    ...
    let broker = Arc::new(RetainedEventBroker::new(remote_address.clone(), agent, outer, outbound, 32));
    ...
    let first = super::super::tests::event(&remote_address, Endpoint::agent(remote_address.clone()),
        1, "application/octet-stream", b"accepted-before-disconnect");
    let second = super::super::tests::event(&remote_address, Endpoint::agent(remote_address.clone()),
        2, "application/octet-stream", b"must-not-overtake");
    let first_digest = event_digest(&encode(&first).unwrap());
    let receipt_key = hop::ReceiptKey { sender_id: "agent-local".into(),
        connection_generation: 7, attempt: 1 };
    remote.receipts.lock().unwrap().reserve(receipt_key.clone(), first_digest,
        &first.envelope.event_id).unwrap();
    remote.receipts.lock().unwrap().commit(&receipt_key, first_digest,
        ReceiptStatus::AcceptedExact, "").unwrap();
    ...
    local.preserve(Failure::HopWriter { value: HopWriteFailure {
        error: io::Error::new(io::ErrorKind::ConnectionReset, "injected receipt loss"),
        state: HopFailureState::Uncertain, current: None,
        outstanding: VecDeque::from([HopOutstanding { attempt: 1, digest: first_digest,
            event: first_owned }]), pending_acks: VecDeque::new(),
        pending: VecDeque::from([second_owned]),
    }, target: Some(remote_address), generation: 7, _slot: Arc::new(slot) });
    ...
    drop(delivered);
    tokio::time::timeout(std::time::Duration::from_secs(2), async {
        while store.storage_snapshot().retained_count != 0 { tokio::task::yield_now().await; }
    }).await.unwrap();
    assert!(local.failures.lock().unwrap().is_empty());
    tokio::time::timeout(std::time::Duration::from_secs(2), async {
        while remote.receipts.lock().unwrap().snapshot().records != 0 { tokio::task::yield_now().await; }
    }).await.unwrap();
...
async fn absent_receipt_quarantines_original_without_replay_or_retirement() {
    ...
    let limits = RuntimeLimits { queue: 2, retained: 4, bytes: 1024 * 1024, connections: 4,
        hop_receipts: 4, hop_receipt_bytes: 1024 * 1024, hop_outstanding: 2 };
    let remote = Shared::with_identity("agent-remote".into(), limits);
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let remote_address = Address::tcp("127.0.0.1", listener.local_addr().unwrap().port());
    let (agent, incoming) = remote.limits.mailbox();
    ...
    let event = super::super::tests::event(&remote_address, Endpoint::agent(remote_address.clone()),
        1, "application/octet-stream", b"uncertain");
    ...
    local.preserve(Failure::HopWriter { value: HopWriteFailure {
        ...
        state: HopFailureState::Uncertain, current: Some(HopOutstanding { attempt: 1,
            digest, event: owned }), outstanding: VecDeque::new(), pending_acks: VecDeque::new(),
    ...
    assert!(matches!(incoming.try_take_owned(), p4_adapter::node_adapter::OwnedPoll::Empty));
    local.tasks.lock().unwrap().abort_all();
    remote.tasks.lock().unwrap().abort_all();
}
...
async fn owned_runtime_transport_owner_keeps_failed_writer_and_queued_originals() {
    ...
    let retained_event_bytes;
    {
        ...
        let Failure::Writer { value, .. } = &mut failures[0].value else { panic!("writer remainder"); };
        ...
        drop(pending);
        retained_event_bytes = p4_adapter::node_adapter::retained_event_bytes(value.current.event()).unwrap();
    }
    ...
    assert_eq!(snapshot["failures"]["retained_event_bytes"], retained_event_bytes);
    assert_eq!(snapshot["failures"]["failure_ids"][0]["failure_id"], "transport-1");
    drop(shared);
    assert_eq!(store.storage_snapshot().retained_bytes, 0);
}
...
async fn owned_runtime_finish_keeps_replacement_route_and_preserves_late_original() {
    ...
    }).await.unwrap();
    {
        ...
        let Failure::Undelivered { event, .. } = &failures[0].value else { panic!("late output must remain owned"); };
...
async fn hop_partial_write_is_uncertain_and_keeps_current_plus_unstarted_queue() {
    ...
    for number in 1..=2 {
        publisher.try_publish_owned(super::super::tests::event(&own,
            Endpoint::agent(own.clone()), number, "application/octet-stream", &[42; 32])).unwrap();
        events.send(next(&store).await.unwrap()).await.unwrap();
    }
    let writes = AtomicU64::new(0);
    let failure = write_hop_loop(&mut BrokenWrite { bytes_left: 11 }, receiver, control_receiver,
        &writes, None, 2, "agent-a", 7).await.unwrap_err();

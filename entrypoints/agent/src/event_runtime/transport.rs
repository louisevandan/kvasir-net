...
impl ConnectionSender {
    ...
    async fn control(&self, frame: p4_protocol::event::hop::HopFrame) -> io::Result<()> {
        self.controls
            .as_ref()
            .ok_or_else(|| io::Error::other("legacy connection has no hop control"))?
            .send(HopCommand::Send(frame))
            .await
            .map_err(|_| io::Error::other("hop control writer closed"))
    }
    async fn received(&self, frame: p4_protocol::event::hop::HopFrame) -> io::Result<()> {
        self.controls
            .as_ref()
            .ok_or_else(|| io::Error::other("legacy connection has no hop control"))?
            .send(HopCommand::Received(frame))
            .await
            .map_err(|_| io::Error::other("hop control writer closed"))
    }
...
impl Shared {
    ...
    fn spawn(&self, future: impl Future<Output = ()> + Send + 'static) {
        ...
        while let Some(result) = tasks.try_join_next() {
            if let Err(error) = result {
                eprintln!("P4_EVENT_TRANSPORT_TASK_FAILED error={error}");
            }
        }
        tasks.spawn(future);
    }
    ...
    fn writer<W: AsyncWrite + Unpin + Send + 'static>(
        self: &Arc<Self>,
        writer: W,
    ...
    fn writer_with_finish<W: AsyncWrite + Unpin + Send + 'static>(
        self: &Arc<Self>,
        mut writer: W,
        slot: Arc<OwnedSemaphorePermit>,
        ...
        self.spawn(async move {
            if let Err(value) = write_loop(&mut writer, receiver, &shared.local_writes).await {
                p4_llamacpp_staged_adapter::v2::record::record(&format!(
                    "P4_EVENT_WRITE_FAILED retained_pending={} result={} error={}",
                    value.pending.len(),
                    if value.started {
                        "uncertain"
                    } else {
    ...
    fn hop_writer<W: AsyncWrite + Unpin + Send + 'static>(
        self: &Arc<Self>,
        mut writer: W,
        slot: Arc<OwnedSemaphorePermit>,
        finish: Option<Arc<AtomicBool>>,
...
impl Inspector {
    #[cfg(test)]
    pub(super) fn detached(limits: RuntimeLimits) -> Self {
        Self(Shared::new(limits))
    }
        ...
        for failure in failures.iter() {
            ...
            *states.entry(state).or_default() += 1;
            retained_event_bytes =
                retained_event_bytes.saturating_add(failure_bytes(&failure.value));
    ...
    match stored.value {
        Failure::Undelivered { error: _, event } => {
            ...
            let sender = match connect(&target, &shared).await {
                Ok(value) => value,
                Err(error) => {
                    stored.value = Failure::Undelivered {
            ...
            shared
                ...
                .lock()
                .await
                .insert(target.clone(), Ok(sender.clone()));
            match sender.send(event).await {
                Ok(()) => Ok((
                    "restarted",
                    "original Event queued on an acknowledged generation".into(),
                )),
                Err(error) => {
                    stored.value = Failure::Undelivered {
                        error: "replacement writer closed".into(),
                        event: error.0,
                    };
        ...
        Failure::HopWriter {
            ...
            for item in value.outstanding.iter() {
                queries.push((item.attempt, item.digest));
            }
            if let Some(item) = &value.current {
                queries.push((item.attempt, item.digest));
            }
            for &(attempt, digest) in &queries {
                let result =
                    match query_receipt(&target, &shared, generation, attempt, digest).await {
                        Ok(value) => value,
                        Err(error) => {
                            value.state = HopFailureState::Unknown;
                            stored.value = Failure::HopWriter {
                                value,
                                target: Some(target),
                                generation,
                                _slot,
                ...
                if result != p4_protocol::event::hop::ReceiptStatus::AcceptedExact {
                    let (state, state_name) = match result {
                        p4_protocol::event::hop::ReceiptStatus::Conflict => {
                            (HopFailureState::Conflict, "conflict")
                        }
                        p4_protocol::event::hop::ReceiptStatus::Rejected => {
                            (HopFailureState::RejectedRemote, "rejected_remote")
                        }
                        p4_protocol::event::hop::ReceiptStatus::Unknown => {
                    ...
                    return Err((
                        stored,
                        state_name,
                        "remote receipt did not prove exact acceptance".into(),
                    ));
                }
            }
            for (attempt, digest) in queries {
                if !value
                    .pending_acks
                    .iter()
                    .any(|intent| intent.attempt == attempt && intent.digest == digest)
                {
                    value
                        .pending_acks
                        .push_back(HopAckIntent { attempt, digest });
                }
            }
            if let Some(item) = value.current.take() {
                item.event.retire();
            }
            for item in value.outstanding.drain(..) {
                item.event.retire();
            }
            if let Err(error) =
                send_receipt_acks(&target, &shared, generation, &value.pending_acks).await
            {
            ...
            Ok((
                "accepted_exact",
                "exact receipts retired predecessors and resumed the original queue".into(),
            ))
        }
        value => {
            stored.value = value;
            ...
            Err((
                stored,
                state,
                "failure is not eligible for explicit replay".into(),
            ))
        }
    }
}
...
fn failure_bytes(failure: &Failure) -> usize {
    ...
    match failure {
        ...
        Failure::HopWriter { value, .. } => value
            .current
            .as_ref()
            .map_or(0, |value| cost(value.event.event()))
            .saturating_add(
                value
                    .outstanding
                    .iter()
                    .map(|value| cost(value.event.event()))
                    .sum(),
            )
            .saturating_add(value.pending.iter().map(|value| cost(value.event())).sum()),
...
impl Owner {
    ...
    pub(super) fn start(
        ...
        outbound: Arc<CompletionMailbox>,
        limits: RuntimeLimits,
    ) -> Self {
    ...
    pub(super) fn inspector(&self) -> Inspector {
        Inspector(Arc::clone(&self.0))
    }
}
impl Drop for Owner {
    fn drop(&mut self) {
...
async fn accept(listener: TcpListener, broker: Arc<RetainedEventBroker>, shared: Arc<Shared>) {
    loop {
        ...
        let Ok(slot) = Arc::clone(&shared.slots).acquire_owned().await else {
            return;
        };
        match listener.accept().await {
            Ok((stream, peer)) => {
                ...
                shared.spawn(async move {
                    serve(id, stream, broker, connection, Arc::new(slot)).await;
                });
            }
...
async fn serve(
    ...
    let first = match read_body(&mut reader).await {
        Ok(Some(value)) => value,
        Ok(None) => {
            eprintln!("P4_EVENT_CONNECTION_STOPPED connection={id} error=FINISH before handshake");
            return;
        }
        Err(error) => {
            eprintln!("P4_EVENT_CONNECTION_STOPPED connection={id} error={error}");
    ...
    match p4_protocol::event::hop::decode(&first) {
        Ok(Some(p4_protocol::event::hop::HopFrame::Hello {
            sender_id,
        ...
        Ok(Some(_)) => {
            eprintln!(
                "P4_EVENT_CONNECTION_STOPPED connection={id} error=first hop frame is not hello"
            );
            return;
        }
        Err(error) => {
        ...
        loop {
            match broker.dispatch_ingress(event) {
                Ok(DispatchOutcome::Duplicate) => {
                    eprintln!("P4_EVENT_DUPLICATE_SUPPRESSED connection={id}");
                    break;
                }
                Ok(_) => break,
                ...
                Err(failure) => {
                    eprintln!(
                        "P4_EVENT_INGRESS_FAILED connection={id} error={}",
                        failure.error
                    );
                    shared.preserve(Failure::Ingress {
...
async fn serve_hop(
    ...
    peer_generation: u64,
    peer_max_outstanding: usize,
) {
    use p4_protocol::event::hop::{HopFrame, ReceiptStatus};
    let finish = Arc::new(AtomicBool::new(false));
    let local_generation = CONNECTIONS.fetch_add(1, Ordering::Relaxed).max(1);
    let max_outstanding = peer_max_outstanding
        .min(shared.limits.hop_outstanding)
        .max(1);
    let sender = shared.hop_writer(
        writer,
        Arc::clone(&slot),
    ...
    if sender
        .control(HopFrame::HelloAck {
            accepted_connection_generation: peer_generation,
            sender_id: shared.sender_id.clone(),
            connection_generation: local_generation,
    ...
    loop {
        ...
        let frame = match p4_protocol::event::hop::decode(&body) {
            Ok(Some(value)) => value,
            Ok(None) => {
                sender.peer_closed("legacy Event after hop hello").await;
                return;
            }
            Err(error) => {
                sender.peer_closed(error.to_string()).await;
        ...
        match frame {
            HopFrame::Data {
                ...
                let key = hop::ReceiptKey {
                    sender_id: peer_sender_id.clone(),
                    connection_generation: peer_generation,
                    attempt,
                };
                ...
                match reservation {
                    ...
                    Err(error) => {
                        let (status, detail) = match error {
                            hop::ReserveError::Conflict => {
                                (ReceiptStatus::Conflict, "attempt digest differs")
                            }
                            hop::ReserveError::Full => {
                                (ReceiptStatus::Rejected, "hop receipt store full")
                            }
                            hop::ReserveError::TooLarge => {
                                (ReceiptStatus::Rejected, "hop receipt exceeds byte limit")
                            }
                        };
                        let _ = sender
                            .control(HopFrame::Receipt {
                ...
                match committed {
                    ...
                    Err(error) => {
                        sender.peer_closed(error).await;
                        return;
                    }
                }
            }
            ...
            HopFrame::Receipt { .. } | HopFrame::QueryResult { .. } => {
                if sender.received(frame).await.is_err() {
                    return;
                }
            }
            HopFrame::Hello { .. } | HopFrame::HelloAck { .. } => {
                sender
                    .peer_closed("hop hello repeated after handshake")
                    .await;
                return;
            }
        }
...
async fn deliver_outer(receiver: Arc<CompletionMailbox>, shared: Arc<Shared>) {
    while let Some(event) = next(&receiver).await {
        ...
        if let Some(event) = failed {
            p4_llamacpp_staged_adapter::v2::record::record(
                "P4_EVENT_OUTER_MISSING retained=1 result=not_started",
            );
            if let Endpoint::Outer(target) = &event.event().envelope.target {
                shared.connections.lock().await.insert(target.clone(), None);
            }
            shared.preserve(Failure::Undelivered {
                error: "OUTER route unavailable".into(),
                event,
            });
        }
    }
    ...
    while let Some(event) = next(&receiver).await {
        ...
        let failure = match peer {
            Ok(sender) => sender
                ...
                .await
                .err()
                .map(|e| ("peer writer closed".into(), e.0)),
...
async fn connect(address: &Address, shared: &Arc<Shared>) -> io::Result<ConnectionSender> {
    let slot = Arc::clone(&shared.slots)
        .acquire_owned()
        .await
    ...
    let response = read_body(&mut reader)
        .await?
        .ok_or_else(|| io::Error::other("peer finished during hop hello"))?;
    let max_outstanding =
        match p4_protocol::event::hop::decode(&response).map_err(io::Error::other)? {
            Some(p4_protocol::event::hop::HopFrame::HelloAck {
                accepted_connection_generation,
                max_outstanding,
                ..
            }) if accepted_connection_generation == generation => (max_outstanding as usize)
                .min(shared.limits.hop_outstanding)
                .max(1),
            Some(p4_protocol::event::hop::HopFrame::HelloAck { .. }) => {
            ...
            None => {
                return Err(io::Error::other(
                    "peer does not support acknowledged hop transport",
                ));
            }
        };
    let sender = shared.hop_writer(
        ...
        loop {
            match read_body(&mut reader).await {
                Ok(Some(body)) => match p4_protocol::event::hop::decode(&body) {
                    Ok(Some(
                        frame @ (p4_protocol::event::hop::HopFrame::Receipt { .. }
                        | p4_protocol::event::hop::HopFrame::QueryResult { .. }),
                    )) => {
                        if reader_sender.received(frame).await.is_err() {
                            return;
                        }
                    }
                    Ok(Some(_)) => {
                        reader_sender
                            .peer_closed("unexpected frame on outbound hop")
                            .await;
                        return;
                    }
                    Ok(None) => {
                        reader_sender
                            .peer_closed("legacy Event on outbound hop")
                            .await;
                        return;
                    }
                    Err(error) => {
                        reader_sender.peer_closed(error.to_string()).await;
                        return;
                    }
                },
                Ok(None) => {
                    reader_sender
                        .peer_closed("unexpected FINISH on outbound hop")
                        .await;
                    return;
                }
                Err(error) => {
...
async fn open_hop_control(address: &Address, shared: &Arc<Shared>) -> io::Result<TcpStream> {
    ...
    let body = read_body(&mut stream)
        .await?
        .ok_or_else(|| io::Error::other("FINISH during reconcile hello"))?;
...
async fn query_receipt(
    ...
    let body = read_body(&mut stream)
        .await?
        .ok_or_else(|| io::Error::other("FINISH before query result"))?;
    let status = match p4_protocol::event::hop::decode(&body).map_err(io::Error::other)? {
        Some(p4_protocol::event::hop::HopFrame::QueryResult {
            attempt: seen_attempt,
            digest: seen_digest,
            status,
            ..
        }) if seen_attempt == attempt && seen_digest == digest => status,
        _ => return Err(io::Error::other("receipt query result identity mismatch")),
    };
    stream.write_u32_le(0).await?;
    stream.flush().await?;
    if stream.read_u32_le().await? != 0 {
        return Err(io::Error::other("invalid reconcile FINISH ACK"));
    }
    Ok(status)
}

async fn send_receipt_acks(
    address: &Address,
    shared: &Arc<Shared>,
    connection_generation: u64,
    intents: &VecDeque<HopAckIntent>,
) -> io::Result<()> {
    if intents.is_empty() {
        return Ok(());
    }
...
async fn write_hop_loop<W: AsyncWrite + Unpin>(
    writer: &mut W,
    ...
    loop {
        if !events_open && outstanding.is_empty() && pending_acks.is_empty() {
            ...
            {
                if let Err(error) = writer.write_u32_le(0).await {
                    return Err(hop_failure(
                        error,
                        HopFailureState::Uncertain,
                        None,
                        outstanding,
                        pending_acks,
                        events,
        ...
        if !controls_open && !outstanding.is_empty() {
            return Err(hop_failure(
                io::Error::new(
                    ...
                    "hop receipt channel closed with outstanding Events",
                ),
                HopFailureState::Uncertain,
                None,
                outstanding,
...
fn drain_pending(
    receiver: &mut mpsc::Receiver<RetainedCompletion>,
) -> VecDeque<RetainedCompletion> {
    receiver.close();
...
async fn write_loop<W: AsyncWrite + Unpin>(
    mut writer: W,
    mut receiver: mpsc::Receiver<RetainedCompletion>,
    local_writes: &AtomicU64,
) -> Result<(), WriteFailure> {
...
async fn write_bytes_frame<W: AsyncWrite + Unpin>(writer: &mut W, bytes: &[u8]) -> io::Result<()> {
    ...
    if bytes.len() > MAX_FRAME {
        return Err(io::Error::other("event frame too large"));
    }

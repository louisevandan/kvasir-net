...
async fn owned_runtime_control_full_retries_and_permanent_failure_retains_inputs_and_reply() {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(10), async {
        while input.storage_snapshot().queued_count != 0 {
    ...
    assert_eq!(
        input.storage_snapshot().retained_count,
        2,
        "active and queued inputs stay charged during Full"
    );
    assert_eq!(input.storage_snapshot().queued_count, 1);
    assert_eq!(
        next(&output).await.unwrap().event().payload,
        b"occupy output"
    );
    for number in [11, 12] {
        let reply = tokio::time::timeout(std::time::Duration::from_secs(10), next(&output))
            .await
            .unwrap()
            .unwrap();
        assert_eq!(
            reply.event().envelope.causation_id,
            Some(format!("input-{number}"))
        );
    }
    ...
    let Some(PendingReply::Owned(reply)) = &stopped.held_reply else {
        panic!("original owned reply must remain");
    };
    assert_eq!(
        reply.event().envelope.causation_id.as_deref(),
        Some("input-13")
    );
    broker
        .dispatch_ingress(event(
...
async fn owned_runtime_delete_refuses_held_completion_even_when_adapter_says_empty() {
    ...
    impl RetainedNodeAdapter for Adapter {
        fn try_offer_retained(&self, event: RetainedCompletion) -> Result<(), RetainedOfferError> {
            Err(RetainedOfferError::Full(event))
        }
        fn peek_retained_completion(&self) -> Option<CompletionFront> {
            self.mailbox.peek_owned_front()
        }
        fn try_take_retained_matching(&self, front: &CompletionFront) -> OwnedPoll {
            self.mailbox.try_take_owned_matching(front)
        }
    ...
    let limits = limits();
    let (agent, _) = limits.mailbox();
    let (outer, _) = limits.mailbox();
    let (outbound, _) = limits.mailbox();
    let broker = Arc::new(RetainedEventBroker::new(
        own.clone(),
        agent,
    ...
    let delete = event(
        ...
        2,
        DELETE,
        b"{\"node_id\":\"node\",\"node_generation\":1}",
    );
    assert!(
        remove(&broker, &mut nodes, &delete)
            .await
            .unwrap_err()
            .contains("delivery must be drained")
    );
    ...
    broker
        .dispatch_ingress(event(
            &own,
            Endpoint::node(own.clone(), "node", 1),
            3,
            "application/octet-stream",
            b"next",
        ))

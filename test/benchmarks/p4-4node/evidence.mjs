...
/// Runs one git command, or fails the run.
///
/// Swallowing the error returned an empty string, and every caller here
/// reads an empty string as "nothing to report": a missing git, a wrong
/// root, or an output over the buffer limit all became a clean working
/// tree. Evidence that cannot be collected is not evidence that there was
/// nothing to collect.
...
/// Writes a captured tree beside the run.
///
/// Takes the capture rather than making its own: computing the identity and
/// then re-reading the tree to write the files leaves a window in which a
/// file can change, and then the identity names something the preserved
/// bytes are not.
///
/// Untracked files are listed rather than copied - one can be a model or a
/// build output, and a run directory is the place for neither - so the
/// manifest tells whether a later tree matches, not how to rebuild this one.
export function preserveWorkingTree(capture, directory) {
  if (capture.clean) return null;
  const written = {};
  if (capture.diff !== "") {
    fs.writeFileSync(path.join(directory, "dirty.diff"), capture.diff, "utf8");
    written.diff = "dirty.diff";
  }
  if (capture.untracked.length > 0) {
    fs.writeFileSync(
      path.join(directory, "untracked.json"),
      `${JSON.stringify(capture.untracked, null, 2)}\n`,
      "utf8",
    );
...
/// One reading of the working tree: its diff, its untracked files, and the
/// identity of both together.
///
/// `id` is null only when the tree is clean. It covers what `git diff HEAD`
/// cannot - binary changes need `--binary`, and a file git has never seen is
/// in no diff at all - because a field that reports null for an
...
export function captureWorkingTree(root) {
  ...
  const untracked = git(root, ["ls-files", "-z", "--others", "--exclude-standard"])
    ...
    .map((name) => {
      ...
      try {
        ...
        // Unreadable is worth recording: the tree held something this run
        // could not account for.
        return { path: name, bytes: null, sha256: null, unreadable: error.message };
      }
    });
  ...
  const id = crypto.createHash("sha256")
    ...
    .update(manifest)
    .digest("hex");
  return { id, clean: false, diff, untracked };
}

export function collectEvidence({ root, runId, spec, compatManifest, remote }) {
  // One reading of the working tree, shared with whatever preserves it: two
  // readings leave a window in which a file changes between the identity and
  // the bytes it is supposed to name.
  const working = captureWorkingTree(root);
  ...
  return {
    ...
    working_tree_id: working.id,
    working_tree: working,
    working_tree_clean: working.clean,
    compat: compat && {

#!/usr/bin/env python3
"""Run the complete local, non-model H0 v2 gate with fixed invocations."""
...
SPEC = DIRECTORY / "benchmark-spec-qwen122b-h0-v2.json"
...
def main() -> None:
    ...
    if json.loads(prepare.stdout) != {"passed": True, "tests": 2}:
        raise RuntimeError("H1 materializer self-test summary differs")
    if json.loads(judge.stdout) != {"passed": True, "tests": 4}:
    ...
    if verified != expected:
        raise RuntimeError("H0 v2 verifier summary differs")
    print(json.dumps({"passed": True, "checks": 6, "inspector_tests": 4,
                      "preflight_tests": 5,
                      "materializer_tests": 2, "judge_tests": 4,

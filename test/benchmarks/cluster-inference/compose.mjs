...
export function compose({ cluster, model, policy, workload, runtime, runId, generation, tokenCounts }) {
  ...
  for (const [name, value] of Object.entries(policy.environment ?? {})) {
    if (!/^P4_STAGED_(MIN_BATCH_ROWS|MAX_OPEN_BATCHES|MAX_ISSUE_ROWS|PREFILL_FRAGMENTS|DECODE_MEMBERS|PREFILL_MEMBERS|PREFILL_ROWS|PREFILL_ROWS_PER_REQUEST)$/.test(name)) throw Error(`unsupported policy field ${name}`);

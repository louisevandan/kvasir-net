...
> 대상: **P4B1 v6** (§90)
> 브랜치: 별도 브랜치에서 진행하며 **종료 시 `apps/linker`와 `apps/p4`만 남는다** (주제 M)
...
| 보완 설계 | `P-1`~`P-59` (철회 2건 포함) |
| 미결 | `Q-1`~`Q-63` 중 **47건 미결**, 16건 결정·철회·해소 |
...
이 작업은 **별도 브랜치**에서 이뤄지며, 브랜치가 끝나면 **`apps/linker`와 `apps/p4`만 남는다.** `apps/llama`는 해체된다.
...
### D-75. `apps/llama` 해체 계획이 없다
브랜치 종료 상태가 정해졌으나 각 구성이 어디로 가는지 미정이다. `P-53`은 upstream·compat·scripts의 이동만 다루고, 웹 UI·supervisor·`llama_domain`은 다루지 않는다.
...
### D-77. OUTER가 모델 형식을 알아야 하는지가 미정이다
D-76의 GGUF 검사가 OUTER로 가면 **OUTER가 GGUF를 안다.** GGUF는 llama.cpp 형식이고, vLLM은 safetensors/HF다. 완결형 백엔드가 늘면 OUTER가 형식마다 검사기를 갖게 된다.

이는 `D-64`(추상 부담이 어디에 실리는가)의 OUTER 측 판본이다. 대안은 어댑터에게 검사를 위임하는 것인데, **편성은 적재 이전에 일어나므로** 그 시점에 어댑터가 그 모델을 들고 있지 않다. 선후 문제가 있다.
...
| 호스트 supervisor (`/api/runtime-groups`) | `apps/p4/adapters/pipeline/` — 어댑터 내부 경계가 된다 |
| 웹 UI (`src/`, vite·tailwind 설정, `assets/`) | `apps/linker`로 흡수하거나 폐기 (Q-60) |
| `backend-contract.json`, `docs/` | 이동처 따라 분산 |

이동이 끝나면 P4의 `apps/llama` 의존이 소멸하고 앱이 둘만 남는다.

### P-58. `packages/llama_domain` 3분할
D-76의 표대로 가른다.
- 배치 휴리스틱·모델 검사 → `packages/linker_domain` (OUTER)
- 런타임 검증·기동 정책 → `apps/p4/adapters/pipeline/`

**11,631줄의 분할이므로 단계로 쪼개야 한다.** `common` 6,113줄이 어느 쪽 공통인지가 실제 작업의 크기를 정한다(Q-61).
...
| Q-60 | `apps/llama`의 웹 UI를 `apps/linker`로 흡수할 것인가 폐기할 것인가 | P-57. 흡수하면 OUTER가 백엔드 운영 화면을 갖는다 |
| Q-61 | `llama_domain`의 `common` 6,113줄은 어느 쪽 공통인가 | P-58. 분할 작업의 실제 크기를 정한다 |
| Q-62 | OUTER가 모델 형식(GGUF·safetensors)을 직접 아는가, 검사를 위임하는가 | D-77. 위임하면 편성 시점에 어댑터가 그 모델을 아직 안 들고 있다는 선후 문제가 있다 |

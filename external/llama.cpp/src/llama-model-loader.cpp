...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    // Whether this linkcpp stage owns the tensor's layer window. A mini-GGUF
    // only carries owned tensors; non-owned ones are physically absent, so we
    // synthesize metadata for them (see below) instead of requiring file data.
    auto linkcpp_stage_owns = [&](const LLM_TN_IMPL & tn_, int flags_) -> bool {
        if (!(linkcpp_layer_begin >= 0 && linkcpp_layer_end >= linkcpp_layer_begin)) {
            return true;
        }
        llm_tensor tn_tensor = tn_.tensor;
        if (tn_.tensor == LLM_TENSOR_TOKEN_EMBD && (flags_ & TENSOR_DUPLICATED)) {
        ...
        if (info.layer == LLM_TENSOR_LAYER_REPEATING) {
            return linkcpp_layer_begin <= tn_.bid && tn_.bid < linkcpp_layer_end;
        } else if (info.layer == LLM_TENSOR_LAYER_INPUT) {
            return linkcpp_layer_begin == 0;
        } else if (info.layer == LLM_TENSOR_LAYER_OUTPUT) {
            return linkcpp_layer_end == (int32_t) hparams.n_layer();
        }
        return true;
    };
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            ...
            if (files.empty() || (linkcpp_layer_begin >= 0 && linkcpp_layer_end >= linkcpp_layer_begin)) {
                // A linkcpp mini-GGUF only carries this stage's window, yet the
                // full graph is still built (and pruned): metadata tensors are
                // synthesized for every non-owned tensor, so reserve full slack.
    ...
    // linkcpp mini-GGUF: a tensor outside this stage's window is absent from
    // the partial file. The full graph is still built and pruned before
    // scheduling, so synthesize a metadata-only tensor (same as the no-file
    // path above). It is skipped at load time (get_weight == nullptr) and never
    // occupies an accelerator/host buffer, so it costs only a ggml_tensor stub.
    if (get_weight(tn.str().c_str()) == nullptr && !linkcpp_stage_owns(tn, flags)) {
        if (flags & TENSOR_NOT_REQUIRED) {
            for (size_t dim = 0; dim < ne.size(); dim++) {
                if (ne.begin()[dim] <= 0) {
                    return nullptr;
        ...
        ggml_tensor t_stub;
        memset(&t_stub, 0, sizeof(ggml_tensor));
        t_stub.type = type;
        for (size_t dim = 0; dim < GGML_MAX_DIMS; dim++) {
            t_stub.ne[dim] = dim < ne.size() ? ne.begin()[dim] : 1;
            GGML_ASSERT(t_stub.ne[dim] >= 1);
            t_stub.nb[dim] = dim == 0 ? ggml_type_size(type) : t_stub.ne[dim-1]*t_stub.nb[dim-1];
            GGML_ASSERT(t_stub.nb[dim] >= 1);
        }
        ggml_set_name(&t_stub, tn.str().c_str());

        ggml_backend_buffer_type_t stub_buft = buft_for_tensor(&t_stub);
        GGML_ASSERT(stub_buft != nullptr);
        ggml_context * stub_ctx = ctx_for_buft(stub_buft);
        ggml_tensor * stub = ggml_dup_tensor(stub_ctx, &t_stub);
        ggml_set_name(stub, tn.str().c_str());
        // Intentionally not counting n_created: the mini-GGUF's n_tensors only
        // covers the owned window; synthesized stubs must not inflate the count
        // (done_getting_tensors would otherwise report "too many tensors").
        return stub;
    }

...
void llm_graph_result::apply_linkcpp_stage(const llm_graph_params & params) {
    ...
    for (int i = 0; i < n_nodes; ++i) {
        ...
        if (begin <= layer && layer < end) {
            ggml_build_forward_expand(gf, nodes[i]);
        }
    }
    if (end < n_layer) {

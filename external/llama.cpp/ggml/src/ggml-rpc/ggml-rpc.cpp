...
static enum ggml_status ggml_backend_rpc_buffer_init_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor) {
    ...
    // Backends such as OpenCL populate tensor->extra for every non-view tensor and
    // need init_tensor called for all of them, so always initialize non-view tensors
    // on the server. (Server persists any resulting extra; see rpc_server::init_tensor.)
...
private:
    ...
    // backend-local per-tensor metadata (e.g. OpenCL's device buffer info) that
    // is populated during init_tensor. The server rebuilds ggml_tensors on every
    // op, so persist extra keyed by the tensor's data pointer and restore it in
    // deserialize_tensor, so backends using tensor->extra can be served over RPC.
...
ggml_tensor * rpc_server::deserialize_tensor(struct ggml_context * ctx, const rpc_tensor * tensor) {
    ...
    // Restore backend-local metadata persisted during init_tensor, so backends
    // that use tensor->extra (e.g. OpenCL) work across RPC ops.
    auto extra_it = tensor_extras.find(tensor->data);
    if (extra_it != tensor_extras.end()) {
        result->extra = extra_it->second;
    }
...
bool rpc_server::set_tensor(const std::vector<uint8_t> & input) {
    ...
    // The backend (e.g. OpenCL) may (re)populate tensor->extra during set_tensor,
    // for example splitting quantized weights into device-side q/scale buffers.
    // Persist the latest extra so compute/get restore the correct one.
    if (tensor->extra != nullptr) {
        tensor_extras[reinterpret_cast<uint64_t>(tensor->data)] = tensor->extra;
    }
    return true;
}
...
bool rpc_server::init_tensor(const rpc_msg_init_tensor_req & request) {
    ...
    if (tensor->extra != nullptr) {
        // The backend (e.g. OpenCL) stored device-local metadata on this tensor.
        // Persist it keyed by the tensor's data pointer so it can be restored on
        // subsequent ops, which rebuild the ggml_tensor via deserialize_tensor.
        tensor_extras[reinterpret_cast<uint64_t>(tensor->data)] = tensor->extra;
    }
...
bool rpc_server::graph_compute(const std::vector<uint8_t> & input) {
    ...
    // Persist backend-local metadata (e.g. OpenCL) for compute-graph tensors, so a
    // later get_tensor on a graph output can restore it (see deserialize_tensor).
    for (int i = 0; i < graph->n_nodes; i++) {
        ggml_tensor * node = graph->nodes[i];
        if (node != nullptr && node->extra != nullptr) {
            tensor_extras[reinterpret_cast<uint64_t>(node->data)] = node->extra;
        }
    }

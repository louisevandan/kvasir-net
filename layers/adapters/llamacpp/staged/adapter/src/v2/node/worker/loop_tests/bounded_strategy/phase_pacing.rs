...
fn authority(state: &AdapterState) -> Authority {
    ...
    Authority {
        ...
        stable: format!("{:?}", (state.next_open_batch, state.next_incarnation,
            requests, &state.flights, &state.open_batches, state.request_budget.used())),
    }
}
...
pub(super) fn assert_timer_authority(interleave_release: bool) {
    ...
    let observer: IssueObserver = Arc::new(move |point, state| {
        ...
        if seen.issued { return; }
        if let Some(peer) = state.requests.values().find(|r| r.command.request_id == "timer-1") {
            seen.peer_slot = peer.sequence_id;
        }
        if point == "after_release_committed" && seen.before.is_some() {
            assert!(interleave_release);
            assert_eq!(seen.release_commits, 0, "only the completed peer may release during this wait");
            let mut expected = seen.before.clone().unwrap();
            expected.free_sequences.push(seen.peer_slot.unwrap());
            assert_eq!(authority(state), expected, "RELEASE only returns its proven peer slot");
            ...
            if let Some(before) = &seen.before { assert_eq!(before, &now); }
            else { seen.before = Some(now); }
            captured_waiting.store(true, Ordering::Release);
        } else if point == "before_native_issue" && seen.before.is_some() {
            assert_eq!(seen.before.as_ref().unwrap(), &authority(state),
        ...
        if point == "decode_coalescing_wait" && interleave_release {
            ...
            while !captured_ack.load(Ordering::Acquire) {
                assert!(Instant::now() < deadline, "genuine release was not delivered");
    ...
    for _ in 0..6 {
        if observation.lock().unwrap().issued { break; }
        ...
        h.until("returned cohort progresses or enters the controlled wait", |h|
            h.held_tail.len() == 4 || (interleave_release && waiting.load(Ordering::Acquire)));
        if interleave_release && waiting.load(Ordering::Acquire) {
            h.until("genuine tail RELEASED is retained", |h| !h.held_control.is_empty());
            assert_eq!(h.held_control.len(), 1);
            let release: ReleaseCommand = serde_json::from_slice(&h.held_control[0].payload).unwrap();
            assert_eq!(release.sequences.len(), 1);
            assert_eq!(release.sequences[0].id, observation.lock().unwrap().peer_slot.unwrap());
            h.pending.extend(h.held_control.drain(..));
            h.tick();
            assert!(h.pending.is_empty(), "release must enter the actual bounded worker input");
            ack_queued.store(true, Ordering::Release);
            h.until("deadline resumes after the real release", |h| h.held_tail.len() == 4);
        }
    }
    let seen = observation.lock().unwrap();
    assert!(seen.issued, "must observe refusal followed by the exact authorized state");
    assert_eq!(seen.release_commits, usize::from(interleave_release));
    drop(seen);
    let calls;
    {

...
class Sha256 final {
public:
    explicit Sha256(bool legacy = false) : legacy_(legacy) {}
...
private:
    // FIPS 180-4 4.2.2. Three entries here (indices 7, 25, 46) previously read
    // ab1c5ed0 / a831c66b / f40e3580 -- each one hex digit off from the
    // correct ab1c5ed5 / a831c66d / f40e3585. That typo made every digest
    // this class ever produced non-standard SHA-256, not just the empty-input
    // case; cross-checked against @noble/hashes (node_modules) and Node's
    // OpenSSL-backed crypto.createHash('sha256'), which agree with each other
    // and with the values below.
    ...
    std::uint32_t round_constant(unsigned index) const {
        if (legacy_) {
            switch (index) {
            case 7: return 0xab1c5ed0;
            case 25: return 0xa831c66b;
            case 46: return 0xf40e3580;
            default: break;
            }
        }
        return k[index];
    ...
    void transform() {
        ...
        for (unsigned i = 0; i < 64; ++i) {
            ...
            const auto t1 = h + s1 + ch + round_constant(i) + w[i];
...
std::string checksum_hex_impl(const std::vector<std::uint8_t> &bytes, bool legacy) {
    Sha256 sha(legacy);
...
std::string checksum_hex(const std::vector<std::uint8_t> &bytes) {
    return checksum_hex_impl(bytes, false);
}

std::string legacy_checksum_hex(const std::vector<std::uint8_t> &bytes) {
    return checksum_hex_impl(bytes, true);
}

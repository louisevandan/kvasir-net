...
bool fail_hop(const char * message, std::string * error) {
    if (error != nullptr) {
        *error = message;
    }
    return false;
...
bool hop_trace_enabled() {
    return std::getenv("P4_STAGED_TRACE_HOP") != nullptr;
}

const char * hop_phase_name(protocol::HopPhase phase) {
...
int32_t ggml_type_from_wire(protocol::WireType type) {
    switch (type) {
...
bool copy_descriptor(const protocol::Descriptor & source,
...
protocol::WireType wire_type_from_ggml(int32_t type) {
    switch (type) {
...
protocol::Descriptor protocol_descriptor(const llama_linkcpp_tensor_desc & source) {

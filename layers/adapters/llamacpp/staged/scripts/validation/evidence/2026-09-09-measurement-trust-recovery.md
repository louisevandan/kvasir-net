...
2. 그래서 96개 요청이 `release_member`와 `issued_work`를 받고도 `released`는 **0개**다.
   **[2026-09-09 정정]** 이 줄은 원래 "해제가 실행된 적이 없다"고 적었다. 그 단정은 틀렸다.
   head는 `worker/effects.rs`의 `CommittedEffect::Release`에서 자기 stage의 해제를 실제로 수행한 뒤
   전달하며, 그 경로는 sequence 하나짜리 개별 검사를 쓰므로 batch 합계 검사에 걸리지 않는다.
   옳은 진술은 **전체 stage의 해제 완료가 확인되지 않았다**이다. 어느 stage가 어디까지 진행했는지는
   이 산출물로 판정할 수 없다.
3. 완료되지 않은 해제만큼 owner·frontier 슬롯이 계속 점유된다.
...
- 거부된 해제 batch의 **실제 폭**은 이 산출물에 기록되지 않는다. 64행 상한은 상수에서 계산한 예측이며,
  관측된 폭으로 확인하지 않았다.
  **[2026-09-09 갱신]** 상한 회계는 실제 wire 명령으로 재어 **63건까지 허용하고 64건부터 거부하는**
  것으로 확정했다(보관된 receipt가 없을 때의 경계). 거부 문구가 member 수와 요구량을 싣도록 고쳤다.
  **그래도 이 실행에서 거부된 명령 종류와 폭은 여전히 미확정이다** — 수정 후 실행이 통과해 확인하지
  못했을 뿐이며, 수정 전 소스의 독립 복사본에 진단 계측을 넣으면 재현은 가능하다.
  상한 회계 자체는 제거했다.
  [수정 증거](2026-09-09-control-receipt-budget.md)

...
fn digest() -> EventDigest {
    event_digest(b"P4E3 canonical event")
}
...
fn every_hop_frame_has_one_canonical_round_trip() {
    let frames = [
        HopFrame::Hello {
            sender_id: "tcp://127.0.0.1:42000".into(),
...
fn legacy_and_malformed_frames_never_cross_classify() {
    ...
    let valid = encode(&HopFrame::ReceiptAck {
        sender_id: "agent-a".into(),
        connection_generation: 7,
        attempt: 1,
        digest: digest(),
...
fn invalid_identity_limits_digest_and_status_are_rejected() {
    ...
    let mut bad_digest = HopFrame::Data {
        attempt: 1,
        digest: digest(),
        event: b"different".to_vec(),
    };
    assert!(encode(&bad_digest).is_err());
    if let HopFrame::Data { digest, event, .. } = &mut bad_digest {
    ...
    let mut receipt = encode(&HopFrame::Receipt {
        attempt: 1,
        digest: digest(),
        status: ReceiptStatus::AcceptedExact,
        detail: String::new(),

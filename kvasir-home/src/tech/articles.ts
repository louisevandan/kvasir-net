...
per token:  backbone → (cur rows, expert ids) → worker → expert partials → backbone`,
  ...
  /* ------------------------------------------------------------------ */
  {
    ...
    dek: "p4 reaches a node by dialing it. Laptops and phones cannot be dialed. The relay is the smallest thing that closes that gap without handing the internet an unauthenticated engine.",
    date: "2026-09-19",
    tags: ["relay", "p4", "desktop"],
    blocks: [
      {
        t: "p",
        md: "The p4 engine finds a node by opening a connection to it. Every stage that finishes its layers hands the result onward by dialing the next stage's agent at the address that agent advertises. It is a clean model and it has one hard edge: **a machine that cannot accept an inbound connection cannot participate.** That is most of the interesting hardware — a laptop behind a home router, a phone behind carrier NAT, a workstation in an office whose firewall is somebody else's afternoon.",
      },
      {
        t: "p",
        md: "We measured that edge rather than assumed it. A desktop node advertising its office address was reachable from inside the building and timed out from anywhere else; the router held the inbound port closed, and opening it was a physical trip and a policy conversation. Every node we actually want — the ones already sitting on people's desks — has the same shape.",
      },
      { t: "h2", kick: "The wrong fix first", text: "Why not just open a port" },
      {
        t: "p",
        md: "Port forwarding works for one machine you control and fails as a network design: it asks each contributor to reconfigure hardware they may not own, and it scales the attack surface with the node count. More seriously, **p4 carries no authentication at all.** It assumes the machines that can reach each other are meant to, which is a reasonable assumption inside a rack and a dangerous one on a public address. Publishing a node is not just a NAT workaround; it hands an unauthenticated engine to the internet.",
      },
      { t: "h2", kick: "The relay", text: "One outbound connection, one signature" },
      {
        t: "p",
        md: "So the relay does two jobs at once. It gives a node an address, and it is the place identity gets proven — precisely because the protocol above it will not do that.",
      },
      {
        t: "code",
        caption: "Framing and handshake.",
...
node ──────── connect ───────▶ relay
     ...
     ◀─────── WELCOME ────────    the node is now reachable`,
      ...
      {
        t: "p",
        md: "The node signs the relay's challenge with **the same keypair its wallet owns**. That is the part worth dwelling on: it makes the node that joins and the wallet that gets paid the same party by construction, rather than by a mapping table somebody has to keep honest. There is no separate node credential to issue, rotate or leak.",
      },
      {
        t: "p",
        md: "Past the handshake the relay is a pipe. It forwards frames without parsing them — it never sees a prompt, a token or a tensor — and it needs no knowledge of the model or of p4's own framing. That is deliberate: the thing holding the trust boundary should be small enough to read in one sitting.",
      },
      { t: "h2", kick: "Shipping it", text: "The agent rides with the app" },
      {
        t: "p",
        md: "None of this helps if joining the network is a second install. The desktop installer now ships the p4 agent alongside the app — a universal binary on macOS, an `.exe` on Windows — so contributing is: install, unlock the wallet, and the node registers and connects with the key that is already there.",
      },
      {
        t: "callout",
        md: "**What this does not yet do.** A node reachable through the relay can join the network; serving *expert shards* from it is a separate piece of work, designed and not yet running. Today's ring runs at layer grain on server hardware.",
      },
    ],
  },
  {

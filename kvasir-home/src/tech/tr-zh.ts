...
export const zhTech: Record<string, TechTranslation> = {
  "a-dialable-address-for-a-laptop": {
    title: "让笔记本也有一个可拨通的地址",
    dek: "p4 通过拨号找到节点派发工作。NAT 之后的机器没有可拨的地址，而大多数机器都在 NAT 之后。让这样一台机器仍能加入，需要什么，以及它加入那天的实测数据。",
    blocks: [
      {
        t: "callout",
        md: "**挡路的不是意愿，而是方向。** p4 通过*向*节点打开 TCP 连接来投递工作。节点自己发起的连接只用于回执 —— 落在其上的 `Data` 帧会被 `peer_closed(\"unexpected frame on outbound hop\")` 断开。笔记本向外打开隧道并从中接收工作，这件事不会发生。靠意愿不行，靠重试不行，靠更好的客户端也不行。",
      },
      { t: "h2", kick: "约束", text: "贡献者的机器默认不可达" },
      {
        t: "p",
        md: "家用机器坐在网络地址转换之后。它能出到互联网，互联网进不来。我们自己的机群同时呈现了两面：办公室的 GB10 直接挂着公网地址，两台 MI250 则在同一个共享出口之后的 `192.168.20.x`。MI250 并不特殊，贡献者的机器正是这个样子。",
      },
      {
        t: "p",
        md: "p4 里没有路由表，没有会合服务，没有打洞。`deliver_outbound` 从事件信封里取出地址并调用 `connect`。就这么多，在机柜里这是合理的设计，只是笔记本无法满足的设计。",
      },
      { t: "h2", kick: "答案", text: "一条向外的连接，一直开着" },
      {
        t: "p",
        md: "中继代节点持有一个公网地址。节点只维持一条通往它的向外连接，自己什么都不监听。当有人拨通那个公网地址，字节就沿着节点早已持有的连接流下来。p4 的两端只看到一个普通地址上的普通套接字，谁都不知道中继的存在。",
      },
      {
        t: "code",
        caption: "拨号方看到的，和节点实际运行的",
        code: "caller  ->  tcp://relay:43100        # 一个普通的 p4 地址\n            |\n            +-- relay      公开。只转发字节，不解析\n                  |\n                  +-- tunnel   节点自己打开的一条向外连接\n                        |\n                        +-- p4-agent  127.0.0.1:42031，只监听回环",
      },
      {
        t: "p",
        md: "中继不读 p4。载荷按字节原样转发、从不解析，所以它分不清 `LOAD` 和 `INSPECT` —— 而且它不该分得清。一旦它理解了流量，它就成了能够改动流量的东西。",
      },
      { t: "h2", kick: "为什么它同时是安全边界", text: "端口转发从来就不是选项" },
      {
        t: "p",
        md: "p4 没有任何认证。没有 TLS，没有令牌，没有白名单 —— 任何能触及代理端口的主机都可以发送 `NODE_LOAD`、`NODE_UNLOAD` 和 `INSPECT`。要求贡献者把家用路由器的端口转发进去是无法辩护的，这正是最显然的解法恰恰是错解的原因。",
      },
      {
        t: "p",
        md: "经由中继，节点什么都不监听。它保持一条向外的连接，并在这条连接承载任何字节之前先证明运营者钱包 —— 用的是结算网关早已使用的同一套 ed25519/base58 签名，于是节点注册的身份就是奖励账本据以记账的身份。可达性与认证，原来是同一个答案。",
      },
      { t: "h2", kick: "生效的那一天", text: "测量，而非宣称" },
      {
        t: "stats",
        items: [
          { n: "6 ms", l: "机群主机到 NAT 后的 Mac" },
          { n: "28 ms", l: "代理应用自身规则所用时间" },
          { n: "512 KiB", l: "逐字节一致地往返" },
          { n: "0", l: "笔记本上开放的端口数" },
        ],
      },
      {
        t: "p",
        md: "数据中心里的一台 MI250 主机拨通 `tcp://34.50.62.159:43100`，在 6 ms 内触达了运行在 NAT 之后一台 MacBook 上的 p4 代理。28 ms 后代理断开了连接，因为第一帧不是 `Hello` —— 这是它自己的协议规则，由它自己执行，而这台机器片刻之前还根本联系不上。那次拒绝就是证据：中继不解析载荷，因此不可能是它做出的。",
      },
      {
        t: "table",
        head: ["检查项", "结果"],
        rows: [
          ["512 KiB 往返", "逐字节一致，177 ms"],
          ["256 KiB 往返", "逐字节一致，108 ms"],
          ["三路并发拨号", "流之间没有串扰"],
          ["未签名的注册", "拒绝"],
          ["重放的挑战值", "拒绝"],
        ],
      },
      { t: "h2", kick: "另一半", text: "节点得有东西可跑" },
      {
        t: "p",
        md: "背后没有引擎，可达性就毫无价值，而桌面应用从未随包携带引擎：它在三个构建目录里找 `p4-agent`，只有在已经手动编译过 p4 的机器上才找得到。现在应用为它所发布的每个平台携带自己的二进制。",
      },
      {
        t: "ul",
        items: [
          "macOS：合并两种架构的 universal 二进制。Mac 应用以 `universal` 打包，资源被原样复制进两个切片，因此只放 arm64 会让 Intel Mac 用户拿到一个看似完整却启动不了节点的应用 —— 一种只在开发者没有的硬件上才显现的故障。",
          "Windows：交叉编译，并让查找去要 `p4-agent.exe`。忘记扩展名，正是 Windows 构建找不到自己代理的那条路。",
          "每次打包都先构建代理，因此不可能产出没有代理的发行版。",
        ],
      },
      {
        t: "callout",
        md: "**还做不到的事。** 节点现在能跑，也能被拨通，但推理仍然不给它带来收益。引擎会发出逐阶段的工作记录，却没有东西去收集；结算网关的贡献度端点 —— 被正确地拦住，使任何节点都无法给自己记账 —— 从未被调用过。运行和获得报酬是两个问题，眼下解开的只是前一个。",
      },
    ],
  },
...
x = x + combine(p, partials) + shared(cur)  # backbone — numerically exact`,
  ...
  "securing-the-kvr-money-path": {
    ...
    blocks: [
      ...
      {
        t: "p",
        md: "Kvasir 的托管模型把密钥留在用户手里：钱包签署交易，Solana 记录交易，结算服务的职责是在触碰任何余额前**验证链上真正发生了什么**。支付遵循 *quote → payment → inference*，每个被消费的交易签名都记入一次性 `usedSignatures` 注册表，永远无法二次出示。这使结算服务成为咽喉要道——它绝不能违背的规则只有一条：只记账链证明的东西，绝不记账客户端声称的东西。不过，该服务在 devnet 上确实持有资金：在链上质押程序上线之前，已质押的 KVR 和预付额度存放在其 treasury 中，并记录在其账本里。",
      },
...
inside the lock:
  ...
  "linkcpp-control-plane": {
    ...
    dek: "推理引擎 自带能干的 RPC 数据平面，却没有控制平面。linkcpp 围绕贴近上游构建的二进制补上缺失的那一半——发现、规划、启动与网关。",
    blocks: [
      {
        t: "p",
        md: "Kvasir 运行的一切都从这里开始。**linkcpp** 是围绕 推理引擎 RPC 数据平面的源码可得控制平面（Business Source License）：它用贴近上游构建的 `ggml-rpc-server` / `llama-server` 二进制在多张 GPU 和多台机器上运行大型 AI 模型。数据平面只带一小组补丁——除此之外，linkcpp 增加的一切都是编排。",
      },
      ...
      { t: "h2", kick: "架构", text: "一个 hub、基于上游的 worker、标准网关" },
      {
        t: "code",
        caption: "请求流——hub 负责编排，基于上游的二进制负责计算。",
        code: `browser / SDK
  ...
  → ggml-rpc-server workers         # local slots, remote units, managed agents`,
      ...
      {
        t: "p",
        md: "这种刻意的分离——源码可得控制平面之下、保持贴近上游的数据平面——是后来一切的地基：环运行时、层市场，直至专家分片蜂群，全都是同一算力之上的控制平面演化。",
      },
    ],
  },
  "ring-topology-pipeline-inference": {
    title: "Phase 1 — 环：无 master 的流水线推理",
    dek: "每台设备只加载自己的层窗口，把小小的 hidden-state 边界传给邻居。没有节点需要持有完整模型，环中也不存在中心 master。",
    blocks: [
      ...
      { t: "h2", kick: "环", text: "层窗口 + 边界传递" },
      {
        ...
        items: [
          ...
          "请求进入环；每个节点运行自己的层，只把 **hidden-state 边界**传给邻居。最后一个 rank 采样 token 并送回——数据路径上没有中心 master，没有节点持有完整模型。",
...
4. MI250 backbone decodes the 122B → every token, layer-0 experts
  ...
  "kvasir-economy-virtuous-cycle": {
    ...
    blocks: [
      ...
      {
        t: "p",
        md: "循环的引擎是 Kvasir 中已经成立的一条规则：**推理必须用 KVR 支付**。这让代币与真实使用绑定——是实用，而非投机。使用为节点所赚的 KVR 提供资金；公平的奖励拉来供给；供给扩大容量，并通过竞争与更细的专家分片压低服务的边际成本；更便宜、更快、更强的服务又拉来更多使用。Kvasir 用一个中心化 API 无法复制的性质让循环更紧：一个参与者可以**同时是消费者和供给者**。需求侧与供给侧往往在*同一群人*身上一起成长，这抑制了那些毁掉单边市场的失衡。",
      },
      ...
      { t: "h2", kick: "不变式", text: "让循环保持良性的三条规则" },
      {
        ...
        items: [
          ...
          "**KVR 是强制媒介。**由于不付 KVR 就无法推理，代币的角色与真实使用而非投机绑定——它是推理的计价单位，而不是一项投资。",
      ...
      {
        t: "p",
        md: "这一切都不需要奇特的机制设计。它需要的是对三件事的自律：奖励来自收入，实用来自使用，平衡来自有界的浮动价格。Kvasir 已经交付了困难而诚实的部分——直接支付到每个节点自有钱包的奖励、与工作成比例的奖励、一个你必须真正花费才能使用网络的代币。其余是经济路线图：逐步收缩、为失败推理的保险池注资的手续费分成，以及恒温器。按这个顺序建成，成本与奖励便不再相争，而开始相互复利。",
      },
    ],
  },
  "remote-gpu-joins-122b": {
    ...
    blocks: [
      ...
      {
        t: "p",
        md: "Kvasir 的前提是*来什么硬件用什么硬件*——包括运营商 NAT 后、公网上、另一座城市里的硬件。Qwen3.5-122B-A10B 把**权重的 86% 装在 12,544 个独立专家里**（49 层 × 256，top-8），每个都是一个 5.3 MB 的纯函数。正是这个颗粒让一台遥远、不相干的机器也能持有一份切片并做出贡献。悬而未决的问题从来不是*能不能拆分*——而是*一个跨越开放互联网的 worker 能否真的参与实时解码，且正确、可问责*。现在它做到了。",
      },
...
per token:  backbone → (cur rows, expert ids) → worker → expert partials → backbone`,
      ...
      {
        t: "p",
        md: "参与若不可问责就毫无价值。中继把**每次会话桥接的字节计量**记入 hub 的贡献账本；网关轮询该账本，把 KVR 增量记到 worker **自己的**钱包。首次跨互联网会话确实累积了：**1.28 MB 的工作 → 1.277952 units → 0.00895 KVR** 的待领奖励。数额很小，而这正是重点——它是真实的、按工作计的结算，不是参与奖杯。",
      },
      {
        t: "p",
  ...
  "wan-dispatch-comm-roadmap": {
    ...
    blocks: [
      ...
      },
      {
        ...
        rows: [
          ["Step-3.7-Flash 428B（今日服务中）", "288 · top-8", "在机队上实测", "是", "服务中 — 16 个 stage，两台机器"],
          ["Qwen3.5-122B", "256 · top-8", "5.3 MB（实测）", "是", "已在 3 台机器上端到端服务"],
          ["GLM-5.2 744B", "—", "—", "是", "已在 linkcpp 上验证 — 报告未公开"],
          ["GLM-4.5 / 4.6 355B", "160 · top-8", "~13 MB", "是", "计划中（钩子已验证）"],
          ["MiniMax-M2 230B", "256 · top-8", "~8 MB", "否", "计划中（钩子已验证）"],
          ["DeepSeek-V3 / R1 671B", "256 · top-8", "~25 MB", "是", "计划中（deepseek2 计算图）"],
          ["Kimi K3 2.8T", "—", "—", "是", "下一道关卡 — 在 p4 上验证"],
          ["Qwen3-235B", "128 · top-8", "~11 MB", "否", "就绪"],
          ["gpt-oss-120b", "128 · top-4", "~14 MB", "否", "就绪"],
          ["Llama 4 Maverick 400B", "128 · top-1", "~70 MB", "是", "计划中（每隔一层是 MoE）"],
      ...
      {
        t: "p",
        md: "整个行业正在向细粒度 MoE 收敛——专家更小、数量更多、稀疏度更高（DeepSeek、Qwen、Kimi、GLM、gpt-oss 都朝这个方向走）。朝那个方向的每一步，都让蜂群的参与单位更小、稀缺市场的颗粒更细。上面这些模型不是愿望清单；每一个都已经流过我们在测试机队上运行的同一个调度钩子——接入是一道验证闸门，而非一个工程项目。",
      },
    ],
  },
  ...
  "moving-the-ring-onto-p4": {
    title: "把环搬到 p4 上",
    dek: "引擎与调用方之间的七处契约变更。每一处的失败方式都不一样，而其中只有一处看起来像是出了错。",
    blocks: [
      {
        t: "p",
        md: "我们合入了 p4 引擎的一个新版本，环就不再服务了。不是崩溃——加载器报告成功，agent 报告 ready，然后什么也没发生。从这份寂静往回倒推花了一天，翻出了**七处**我们的调用方与引擎已经各走各路的地方。值得写下来的不是这个数字，而是七处里有六处根本没有报错。",
      },
      { t: "h2", kick: "失败其一", text: "指向不存在节点的事件会被转发，而不是被拒绝" },
      {
        t: "p",
        md: "我们的加载器把 LOAD 命令发给了它想要创建的那个节点。但在 LOAD 创建它之前，节点并不存在，而 broker 对一个指名未知节点的事件的规则是**把它向外转发**，而不是拒绝。于是这条命令离开 agent 去别处找归宿，没找到，被丢弃。没有任何日志，因为在 broker 看来什么错也没发生。",
      },
      {
        t: "p",
        md: "修法是把 LOAD 发给那个 *agent*，包在引擎与后端无关的生命周期信封里，适配器自己的命令作为不透明的载荷。事后看来显而易见；从外面看却完全不可见。",
      },
      { t: "h2", kick: "失败其二", text: "一个必须等于另一个数字的数字" },
      {
        t: "p",
        md: "一个模型在某个 **load generation** 下被加载，而每个节点以某个 **node generation** 注册。我们一直把这两者当作互不相干——一个用时间戳，另一个写 `1`——而一切都正常。环加载了。它正确回答了一次请求。然后头部节点死了。",
      },
      {
        t: "code",
        caption: "适配器的 release 记账里的那个检查。",
...
if *generation != receipt.load_generation {
    return Err("release owner census generation differs from source");
      ...
      {
        t: "p",
        md: "结束一次请求的那张 receipt 携带着 load generation，而发出它的节点携带着自己的那个。两者不一致，节点就被停掉。所以这个 bug 的形状是：**加载成功、首次请求成功、头部死亡、之后每一次会话都半加载地挂住。**它读起来完全像负载下的崩溃，一点也不像一次数字不匹配。现在我们的加载器会在任何东西被加载之前，就拒绝两个数字不一致的方案。",
      },
      { t: "h2", kick: "失败其三", text: "一个永远不可能被满足的上界" },
      {
        t: "p",
        md: "适配器会把一个 stage 所能返回的最大结果，与 stage server 启动时报出的数字按完全相等比对。而不加载模型我们就无从知道那个数字——于是我们带着一个猜测去加载，失败一次性把四个 stage 的真实数字全告诉了我们：",
      },
      {
        t: "code",
        caption: "一次运行，四个答案。",
...
step37-s3: profile=33554432, READY=59136012`,
      ...
      {
        t: "p",
        md: "34 GB。agent 的保留存储是 256 MiB，所以那个环根本不可能被准入。把 stage server 里的推导读一遍就明白了：这个上界随 `n_batch × n_ubatch` 增长，而我们从一份早于这项检查的配置里继承了 2048 的批宽。在 128 行——生产布局所用的宽度——上界是 138 MB，放得下。现在我们在方案里用同一个公式推导它，而不是抱着一个记住的常数；尾部 stage 的预测值与另一套部署记录下来的数字分毫不差，而这种一致，正是你在花二十分钟去加载之前最想要的。",
      },
      { t: "h2", kick: "另外四处", text: "简短地说" },
      {
        t: "ul",
        items: [
          "**双主机环里的 loopback 地址。**一个 stage 按下一个 stage 的 agent 所公示的地址拨过去。公示 `127.0.0.1`，主机 A 就拨给了自己。那个环记录在案的配置一直都是 loopback——它跨主机从来就不可能工作。",
          "**日志是强制的。**没有 agent 的运行日志（operational journal），模型不会加载。我们在追另一个错误时把它关掉了，结果让症状变得更糟，而且糟得像是有了进展。",
          "**设备名是分后端的。**参考方案生成器面向 CUDA，发出 `--device CUDA0`。而 HIP 构建把自己的设备叫作 `ROCm0`。那份方案会先把整个模型加载完，*然后*才找不到设备。",
          "**原生 server 是发行版的一部分。**用更新的代码树构建的 agent 想要的能力，已安装的 stage server 并不上报。同样是在一次完整的模型加载之后才被发现。",
        ],
      },
      { t: "h2", kick: "我们从中学到的", text: "寂静才是昂贵的失败模式" },
      {
        t: "p",
        md: "这些每一个都便宜到好修、昂贵到难找，而且模式一致：真正昂贵的失败，都是那种看起来一切正常的系统什么也没做、或者只做了一次。我们加上的守卫全是同一个形状——尽早拒绝，就在错误还看得懂的那个地方。加载器在第一条命令发出*之前*就把 load generation 写到磁盘上，因为否则它无从找回。它直接拒绝 generation 不匹配，而不是等第一次请求之后才发现。它推导结果上界，而不是记住它。",
      },
      {
        t: "callout",
        md: "**环正在服务。**两台机器上的四个 stage，113 GiB 权重常驻，冷启动首 token 1.4 s、预热后约 0.3 s，而每节点的贡献第一次一路流进了结算账本。",
      },
    ],
  },
  "the-template-is-the-callers-job": {
    title: "模板是调用方的活",
    dek: "p4 转发不透明的提示词，不套用任何对话模板。忘了这一点，模型就会流畅地回答一个你没问过的问题——一路答到 token 上限。",
    blocks: [
      {
        t: "p",
        md: "我们恢复过来的那个环给出的第一个真正答案，是正确的算术，后面跟着一场没人参与过的对话：",
      },
      {
        t: "code",
        caption: "向一个已服务的模型提问 17 × 23。",
        code: `" 391\n\nWhat is 12 times 12? Reply with only the number. 144\n\nWhat is 14"`,
      },
      {
        t: "p",
        md: "数字是对的。它后面的一切都是模型在续写一份文档，因为我们递给它的正是这个：被摊平成一个字符串的消息列表。instruct 模型把它读成一段要续写的文本，而不是一个要回答的轮次。它永远不会发出自己的轮次结束 token，于是每一次生成都跑到上限。",
      },
      { t: "h2", kick: "谁的活", text: "这是刻意的省略，不是缺口" },
      {
        t: "p",
        md: "p4 把不透明的提示词原样交给 stage server，自己不套用任何轮次格式——分阶段适配器只带了一个从 GGUF 里*读出*模板的工具，从来没有套用它的工具。这条线画得有道理：引擎保持窄小、与模型无关，而本就知道自己在对哪个模型说话的调用方去渲染格式。但一条画了却没写进文档的线，总会有人踩过去。",
      },
      {
        t: "p",
        md: "把模板从模型文件里读出来，事情就清楚了：ChatML 轮次、以 `<|im_end|>` 作为轮次结束 token，以及一个以思考块开场的 assistant 轮次。让 bridge 渲染这些之后，同样的问题：",
      },
      {
        t: "code",
        caption: "同一个模型、同一个环，套用了轮次格式。",
        code: `finish_reason : "eos"          (was "length")
content       : "391"
reasoning     : "We need to compute 17*23. 17*20=340, plus 17*3=51, total 391."`,
      },
      { t: "h2", kick: "真正花钱的那部分", text: "一次思考可以把答案吃掉" },
      {
        t: "p",
        md: "推理模型在开口之前先花 token。给它一个预算和一道难题，它可能把整个预算都花在思考上，留下一个空答案——而在一个调用方**在请求运行之前就已经在链上付过钱**的网络里，空答案不是质量问题，是收了钱什么也没给。",
      },
      {
        t: "p",
        md: "结算网关早就知道这件事，会要求关掉思考。模型的模板里没有这个开关，于是 bridge 在提示词里把思考块*打开又关上*，模型接在后面写答案。我们在这里以最显而易见的方式错过一次：在提示词里闭合思考块，意味着闭合标签不再出现在输出里，于是分离器把整个答案都归成了 reasoning，返回了空的 content。而这恰恰是那个设置本来要防的失败。",
      },
      {
        t: "callout",
        md: "**这让契约落在哪里。**引擎转发字节。bridge 知道模型：它渲染放置方案里指名的轮次格式，把思考过程作为 `reasoning_content` 与 `content` 分开返回，并把要求输出超过环所加载容量的请求夹到上限——因为过大的请求否则会被直接拒绝，而一个短一点的答案总好过一个引擎错误。",
      },
    ],
  },
};

...
per token:  backbone → (cur rows, expert ids) → worker → expert partials → backbone`,
  ...
  "a-dialable-address-for-a-laptop": {
    title: "给一台笔记本一个可拨号的地址",
    dek: "p4 靠拨号触达一个节点。笔记本和手机没法被拨号。中继是补上这道缺口、又不把一台无认证引擎交给互联网的最小方案。",
    blocks: [
      {
        t: "p",
        md: "p4 引擎通过向一个节点发起连接来找到它。每个跑完自己那段层的 stage，都要按下一个 stage 的 agent 所公示的地址拨过去，才能把结果递交出去。这是个干净的模型，但它有一道硬边：**无法接受入站连接的机器就无法参与。**而那恰恰是大多数有意思的硬件——家用路由器后的笔记本、运营商 NAT 后的手机、防火墙归别人管上一下午的办公室工作站。",
      },
      {
        t: "p",
        md: "这道边我们是实测出来的，不是假设出来的。一个公示了办公室地址的桌面节点，在楼里可达，从任何其他地方都超时；路由器把入站端口关着，而打开它意味着跑一趟现场外加一场政策沟通。每一台我们真正想要的节点——那些已经摆在人们桌上的机器——都是同一副形状。",
      },
      { t: "h2", kick: "先说错误的解法", text: "为什么不干脆开个端口" },
      {
        t: "p",
        md: "端口转发对一台你自己说了算的机器有效，作为网络设计却失败：它要求每位贡献者去重新配置未必属于自己的硬件，而攻击面会随节点数一起增长。更要紧的是，**p4 完全不带认证。**它假定彼此可达的机器就是应该彼此可达的——这在机架内部是合理假设，在一个公网地址上则是危险假设。把一个节点发布出去，不只是绕过 NAT；那是把一台无认证的引擎交给互联网。",
      },
      { t: "h2", kick: "中继", text: "一条出站连接，一次签名" },
      {
        t: "p",
        md: "所以中继同时干两件事。它给节点一个地址，同时也是身份被证明的地方——恰恰因为它之上的协议不会做这件事。",
      },
      {
        t: "code",
        caption: "帧格式与握手。",
...
node ──────── connect ───────▶ relay
     ...
     ◀─────── WELCOME ────────    the node is now reachable`,
      ...
      {
        t: "p",
        md: "节点用**其钱包所拥有的同一把密钥对**签署中继的挑战。这一点值得多停留一会儿：它让\"加入的那个节点\"和\"收钱的那个钱包\"在构造上就是同一方，而不是靠一张要有人一直维护得诚实的映射表。没有另一套节点凭据需要签发、轮换或泄漏。",
      },
      {
        t: "p",
        md: "握手之后，中继只是一根管道。它转发帧而不解析——永远看不到提示词、token 或张量——也不需要知道模型，甚至不需要知道 p4 自己的帧格式。这是刻意的：持有信任边界的那个东西，应该小到一口气就能读完。",
      },
      { t: "h2", kick: "把它发出去", text: "agent 随应用一起上路" },
      {
        t: "p",
        md: "如果加入网络需要第二次安装，上面这些就都白搭。桌面安装包现在把 p4 agent 与应用一起装上——macOS 上是通用二进制，Windows 上是一个 `.exe`——于是贡献算力就只剩：安装、解锁钱包，节点便用那把本就在那里的密钥完成注册并连接。",
      },
      {
        t: "callout",
        md: "**它还做不到什么。**能经中继访问的节点可以加入网络；从它上面提供*专家分片*是另一件事，已完成设计、尚未运行。今天的环在服务器硬件上以层为粒度运行。",
      },
    ],
  },

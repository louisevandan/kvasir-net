...
per token:  backbone → (cur rows, expert ids) → worker → expert partials → backbone`,
  ...
  "a-dialable-address-for-a-laptop": {
    title: "노트북에 dial 가능한 주소를",
    dek: "p4는 노드를 dial해서 닿는다. 노트북과 폰은 dial할 수 없다. 릴레이는 인증 없는 엔진을 인터넷에 넘기지 않으면서 그 간극을 메우는 가장 작은 장치다.",
    blocks: [
      {
        t: "p",
        md: "p4 엔진은 노드에 연결을 열어 노드를 찾는다. 자기 레이어를 마친 스테이지는 모두 다음 스테이지의 에이전트를 그 에이전트가 광고하는 주소로 dial해 결과를 넘긴다. 깔끔한 모델이지만 날카로운 모서리가 하나 있다: **인바운드 연결을 받을 수 없는 머신은 참여할 수 없다.** 그런데 흥미로운 하드웨어의 대부분이 여기 해당한다 — 홈 라우터 뒤의 노트북, 캐리어 NAT 뒤의 폰, 방화벽이 남의 오후 일거리인 사무실의 워크스테이션.",
      },
      {
        t: "p",
        md: "이 모서리는 가정하지 않고 측정했다. 사무실 주소를 광고하는 데스크톱 노드는 건물 안에서는 닿았고 밖에서는 전부 타임아웃이었다. 라우터가 인바운드 포트를 닫아 두고 있었고, 그 포트를 여는 일은 현장 방문 한 번과 정책 논의 한 번이었다. 우리가 실제로 원하는 노드 — 이미 사람들 책상 위에 있는 그 노드들 — 은 전부 같은 모양이다.",
      },
      { t: "h2", kick: "첫 번째 오답", text: "그냥 포트를 열면 안 되나" },
      {
        t: "p",
        md: "포트 포워딩은 내가 통제하는 머신 한 대에서는 동작하지만 네트워크 설계로는 실패한다: 기여자 한 명 한 명에게 자기 소유가 아닐 수도 있는 하드웨어를 재설정하라고 요구하고, 공격면이 노드 수에 비례해 커진다. 더 심각한 것은 이쪽이다: **p4에는 인증이 전혀 없다.** 서로 닿을 수 있는 머신들은 닿아도 되는 사이라고 가정하는데, 랙 안에서는 합리적이고 공개 주소에서는 위험한 가정이다. 노드를 공개하는 것은 단순한 NAT 우회가 아니라, 인증 없는 엔진을 인터넷에 건네는 일이다.",
      },
      { t: "h2", kick: "릴레이", text: "아웃바운드 연결 하나, 서명 하나" },
      {
        t: "p",
        md: "그래서 릴레이는 두 가지 일을 동시에 한다. 노드에 주소를 주고, 그리고 — 바로 위 계층의 프로토콜이 그것을 하지 않기 때문에 — 정체성이 증명되는 자리가 된다.",
      },
      {
        t: "code",
        caption: "프레이밍과 핸드셰이크.",
...
node ──────── connect ───────▶ relay
     ...
     ◀─────── WELCOME ────────    the node is now reachable`,
      ...
      {
        t: "p",
        md: "노드는 릴레이의 challenge를 **자기 지갑이 소유한 바로 그 키페어**로 서명한다. 곱씹을 만한 대목이다: 합류하는 노드와 보상받는 지갑이, 누군가 정직하게 관리해야 하는 매핑 테이블이 아니라 구조적으로 같은 주체가 된다. 따로 발급하고 교체하고 유출될 수 있는 노드 자격증명이 아예 없다.",
      },
      {
        t: "p",
        md: "핸드셰이크를 지나면 릴레이는 파이프다. 프레임을 파싱하지 않고 그대로 전달하므로 — 프롬프트도, 토큰도, 텐서도 보지 않는다 — 모델도 p4 자신의 프레이밍도 알 필요가 없다. 의도적이다: 신뢰 경계를 쥔 물건은 한자리에 앉아 다 읽을 수 있을 만큼 작아야 한다.",
      },
      { t: "h2", kick: "출시", text: "에이전트가 앱과 함께 실린다" },
      {
        t: "p",
        md: "네트워크 합류가 두 번째 설치라면 이 모든 게 소용없다. 이제 데스크톱 설치 프로그램이 앱과 함께 p4 에이전트를 싣는다 — macOS에서는 universal binary, Windows에서는 `.exe` — 그래서 기여는 이렇게 된다: 설치하고, 지갑을 잠금 해제하면, 이미 거기 있는 키로 노드가 등록되고 접속한다.",
      },
      {
        t: "callout",
        md: "**아직 되지 않는 것.** 릴레이로 닿는 노드는 네트워크에 합류할 수 있지만, 거기서 *전문가 샤드*를 서빙하는 것은 별개의 작업이고 설계는 끝났으나 아직 돌지 않는다. 오늘의 링은 서버 하드웨어에서 레이어 단위로 돈다.",
      },
    ],
  },

...
export const koTech: Record<string, TechTranslation> = {
  "a-dialable-address-for-a-laptop": {
    title: "노트북에도 걸 수 있는 주소를",
    dek: "p4는 노드에게 걸어서 일을 준다. NAT 뒤의 기계에는 걸 주소가 없다. 그리고 대부분의 기계가 NAT 뒤에 있다. 그런 기계 하나를 합류시키기까지 무엇이 필요했는지, 그리고 합류한 날의 실측이다.",
    blocks: [
      {
        t: "callout",
        md: "**막힌 것은 의지가 아니라 방향이었다.** p4는 노드*에게* TCP 연결을 열어 일을 전달한다. 노드가 스스로 연 연결은 영수증 전용이라, 그 위로 온 `Data` 프레임은 `peer_closed(\"unexpected frame on outbound hop\")`으로 끊긴다. 노트북이 바깥으로 터널을 열고 그 터널로 일감을 받는 일은 일어나지 않는다. 의지로도, 재시도로도, 더 나은 클라이언트로도.",
      },
      { t: "h2", kick: "제약", text: "기여자의 기계는 기본적으로 도달 불가다" },
      {
        t: "p",
        md: "가정의 기계는 NAT 뒤에 앉아 있다. 인터넷으로 나갈 수는 있지만 인터넷이 들어올 수는 없다. 우리 플릿이 양쪽을 다 보여준다. 사무실 GB10은 공인 주소가 직접 붙어 있고, MI250 두 대는 하나의 공용 출구 뒤 `192.168.20.x`에 있다. MI250이 특이한 게 아니다. 기여자의 기계가 바로 그 모습이다.",
      },
      {
        t: "p",
        md: "p4에는 라우팅 테이블도, 랑데부 서비스도, 홀펀칭도 없다. `deliver_outbound`는 이벤트 봉투에서 주소를 꺼내 `connect`를 부른다. 그게 전부이고, 랙 안에서는 합리적인 설계다. 다만 노트북이 만족시킬 수 있는 설계가 아닐 뿐이다.",
      },
      { t: "h2", kick: "해법", text: "바깥으로 연 연결 하나를 붙들고 있기" },
      {
        t: "p",
        md: "릴레이가 노드를 대신해 공인 주소를 갖는다. 노드는 그쪽으로 나가는 연결 하나만 유지하고 아무것도 듣지 않는다. 누군가 그 공인 주소로 걸면 바이트는 노드가 이미 들고 있던 연결을 타고 내려온다. p4의 양쪽 끝은 평범한 주소로 향하는 평범한 소켓을 볼 뿐, 어느 쪽도 릴레이의 존재를 알지 못한다.",
      },
      {
        t: "code",
        caption: "거는 쪽이 보는 것과 노드가 실제로 돌리는 것",
        code: "caller  ->  tcp://relay:43100        # 평범한 p4 주소\n            |\n            +-- relay      공개. 바이트를 넘기되 해석하지 않는다\n                  |\n                  +-- tunnel   노드가 스스로 연 바깥 연결 하나\n                        |\n                        +-- p4-agent  127.0.0.1:42031, 루프백만 듣는다",
      },
      {
        t: "p",
        md: "릴레이는 p4를 읽지 않는다. 페이로드를 바이트 단위로 넘기고 파싱하지 않으므로 `LOAD`와 `INSPECT`를 구분하지 못한다 — 그리고 구분할 수 있으면 안 된다. 트래픽을 이해하는 순간 그것을 바꿀 수 있는 물건이 되기 때문이다.",
      },
      { t: "h2", kick: "이것이 동시에 보안 경계인 이유", text: "포트포워딩은 애초에 선택지가 아니었다" },
      {
        t: "p",
        md: "p4에는 어떤 인증도 없다. TLS도, 토큰도, 허용목록도 없어서 에이전트의 포트에 닿는 누구나 `NODE_LOAD`·`NODE_UNLOAD`·`INSPECT`를 보낼 수 있다. 기여자에게 집 공유기의 포트를 그쪽으로 열라고 요구하는 것은 변호할 수 없고, 가장 뻔한 해법이 틀린 해법인 이유가 그것이다.",
      },
      {
        t: "p",
        md: "릴레이를 거치면 노드는 아무것도 듣지 않는다. 바깥으로 연 연결 하나를 유지하고, 그 연결이 바이트를 나르기 전에 운영자 지갑을 증명한다 — 정산 게이트웨이가 이미 쓰는 것과 같은 ed25519/base58 서명이라, 노드가 등록하는 신원이 보상 원장이 키로 쓰는 신원과 같다. 도달성과 인증은 결국 같은 답을 갖는다.",
      },
      { t: "h2", kick: "동작한 날", text: "주장이 아니라 측정" },
      {
        t: "stats",
        items: [
          { n: "6 ms", l: "플릿 호스트에서 NAT 뒤 Mac 까지" },
          { n: "28 ms", l: "에이전트가 자기 규칙을 적용하기까지" },
          { n: "512 KiB", l: "바이트 단위로 동일하게 왕복" },
          { n: "0", l: "노트북에서 연 포트 수" },
        ],
      },
      {
        t: "p",
        md: "데이터센터의 MI250 호스트가 `tcp://34.50.62.159:43100`으로 걸어 NAT 뒤 맥북에서 도는 p4 에이전트에 6 ms 만에 닿았다. 28 ms 뒤 에이전트가 연결을 끊었다. 첫 프레임이 `Hello`가 아니었기 때문이다 — 자기 프로토콜 규칙을, 조금 전까지 연락조차 닿지 않던 기계가 스스로 적용한 것이다. 그 거절이 증거다. 릴레이는 페이로드를 파싱하지 않으므로 그것을 만들어낼 수 없다.",
      },
      {
        t: "table",
        head: ["검사", "결과"],
        rows: [
          ["512 KiB 왕복", "바이트 동일, 177 ms"],
          ["256 KiB 왕복", "바이트 동일, 108 ms"],
          ["동시 발신자 3개", "스트림이 서로 섞이지 않음"],
          ["서명 없는 등록", "거부"],
          ["재사용된 챌린지", "거부"],
        ],
      },
      { t: "h2", kick: "나머지 절반", text: "노드에는 돌릴 것이 있어야 한다" },
      {
        t: "p",
        md: "엔진이 뒤에 없으면 도달성은 무의미한데, 데스크탑 앱은 그것을 배포한 적이 없었다. 빌드 디렉터리 세 곳에서 `p4-agent` 바이너리를 찾았고, 이미 p4를 손수 컴파일한 기계에서만 발견됐다. 이제 앱이 배포되는 플랫폼마다 자기 바이너리를 싣는다.",
      },
      {
        t: "ul",
        items: [
          "macOS: 두 아키텍처를 합친 universal 바이너리. Mac 앱은 `universal`로 패킹되고 리소스는 양쪽 슬라이스에 그대로 복사되므로, arm64만 넣으면 Intel Mac 사용자는 멀쩡해 보이는데 노드가 켜지지 않는 앱을 받는다 — 개발자가 가진 적 없는 하드웨어에서만 보이는 실패다.",
          "Windows: 크로스 컴파일하고, 탐색이 `p4-agent.exe`를 찾도록 했다. 확장자를 빼먹는 것이 윈도우 빌드가 자기 에이전트를 못 찾는 경로다.",
          "패키징마다 에이전트 빌드가 먼저 돌므로, 에이전트 없는 릴리스는 만들어질 수 없다.",
        ],
      },
      {
        t: "callout",
        md: "**아직 되지 않는 것.** 노드는 이제 돌고 도달된다. 다만 추론으로 버는 것은 여전히 없다. 엔진은 스테이지별 작업 기록을 내보내지만 그것을 모으는 것이 없고, 정산 게이트웨이의 기여도 엔드포인트는 — 어떤 노드도 자기 기여를 스스로 적을 수 없도록 올바르게 막혀 있는 채로 — 한 번도 호출된 적이 없다. 도는 것과 보상받는 것은 별개의 문제이고, 지금 풀린 것은 앞의 하나뿐이다.",
      },
    ],
  },
...
x = x + combine(p, partials) + shared(cur)  # backbone — numerically exact`,
  ...
  "securing-the-kvr-money-path": {
    ...
    blocks: [
      ...
      {
        t: "p",
        md: "Kvasir의 수탁 모델은 키를 사용자에게 둔다: 지갑이 트랜잭션에 서명하고, Solana가 이를 기록하며, 정산 서비스의 일은 잔액을 건드리기 전에 **체인에서 실제로 일어난 일을 검증**하는 것이다. 결제는 *quote → payment → inference*를 따르고, 소비된 모든 트랜잭션 서명은 일회용 `usedSignatures` 레지스트리에 기록돼 두 번 제시될 수 없다. 그래서 정산 서비스가 관문이 되고 — 절대 어겨선 안 되는 규칙은 하나다: 체인이 증명한 것만 적립하고, 클라이언트가 주장한 것은 절대 적립하지 않는다. 다만 이 서비스는 devnet에서 자금을 보관한다: 온체인 스테이킹 프로그램이 출시될 때까지 스테이킹된 KVR과 선불 크레딧은 서비스의 트레저리에 있고 원장으로 관리된다.",
      },
...
inside the lock:
  ...
  "linkcpp-control-plane": {
    ...
    dek: "추론엔진에는 유능한 RPC 데이터 플레인은 있지만 컨트롤 플레인이 없다. linkcpp가 그 나머지 절반 — 탐색·계획·실행·게이트웨이 — 을 업스트림에 가깝게 빌드한 바이너리 둘레에 더한다.",
    blocks: [
      {
        t: "p",
        md: "Kvasir가 돌아가는 모든 것은 여기서 시작한다. **linkcpp**는 추론엔진의 RPC 데이터 플레인을 감싸는 소스 공개 컨트롤 플레인(Business Source License)이다: 업스트림에 가깝게 빌드한 `ggml-rpc-server` / `llama-server` 바이너리로 대형 AI 모델을 여러 GPU와 머신에 걸쳐 실행한다. 데이터 플레인에는 소규모 패치 세트만 들어간다 — 그 밖에 linkcpp가 더하는 것은 전부 오케스트레이션이다.",
      },
      ...
      { t: "h2", kick: "아키텍처", text: "하나의 허브, 업스트림 기반 워커, 표준 게이트웨이" },
      {
        t: "code",
        caption: "요청 흐름 — 허브가 조율하고, 업스트림 기반 바이너리가 계산한다.",
        code: `browser / SDK
  ...
  → ggml-rpc-server workers         # local slots, remote units, managed agents`,
      ...
      {
        t: "p",
        md: "이 의도적 분리 — 소스 공개 컨트롤 플레인 아래의, 업스트림에 가깝게 유지된 데이터 플레인 — 가 이후 모든 것의 토대다: 링 런타임, 레이어 마켓, 그리고 결국 전문가 샤딩 스웜까지, 전부 같은 컴퓨트 위의 컨트롤 플레인 진화다.",
      },
    ],
  },
  "ring-topology-pipeline-inference": {
    title: "Phase 1 — 링: 마스터 없는 파이프라인 추론",
    dek: "모든 기기는 자기 레이어 구간만 로드하고 작은 hidden-state 경계를 이웃에 넘긴다. 어떤 노드도 모델 전체를 가질 필요가 없고, 링에는 중앙 마스터가 없다.",
    blocks: [
      ...
      { t: "h2", kick: "링", text: "레이어 구간 + 경계 전달" },
      {
        ...
        items: [
          ...
          "요청이 링에 들어오면 각 노드는 자기 레이어를 실행하고 **hidden-state 경계**만 이웃에 넘긴다. 마지막 rank가 토큰을 샘플링해 되돌려보낸다 — 데이터 경로에 중앙 마스터 없음, 어떤 노드도 모델 전체를 갖지 않음.",
...
4. MI250 backbone decodes the 122B → every token, layer-0 experts
  ...
  "kvasir-economy-virtuous-cycle": {
    ...
    blocks: [
      ...
      {
        t: "p",
        md: "이 순환의 엔진은 Kvasir에서 이미 참인 단 하나의 규칙이다: **추론은 반드시 KVR로 지불된다**. 그래서 토큰이 실제 사용에 묶인다 — 투기가 아니라 유틸리티다. 사용이 노드가 버는 KVR의 재원이 되고, 공정한 보상이 공급을 끌어오고, 공급이 용량을 키우며 경쟁과 더 세밀한 전문가 샤딩을 통해 서빙의 한계 비용을 낮추고, 더 싸고 빠르고 유능한 서비스가 더 많은 사용을 끌어온다. Kvasir는 어떤 중앙화 API도 복제할 수 없는 성질로 루프를 더 조인다: 참여자가 **소비자이자 공급자를 동시에** 될 수 있다는 점이다. 수요 측과 공급 측이 흔히 *같은 사람들* 안에서 자라며, 일면 시장을 망가뜨리는 불균형을 완화한다.",
      },
      ...
      { t: "h2", kick: "불변식", text: "순환을 선하게 유지하는 세 규칙" },
      {
        ...
        items: [
          ...
          "**KVR은 필수 매개다.** KVR을 내지 않고는 추론할 수 없으므로, 토큰의 역할은 투기가 아니라 실제 사용에 묶인다 — KVR은 추론의 계산 단위이지 투자 대상이 아니다.",
      ...
      {
        t: "p",
        md: "이 중 어느 것도 별난 메커니즘 설계를 요구하지 않는다. 세 가지에 대한 규율을 요구할 뿐이다: 수익에서 나오는 보상, 사용에서 나오는 유틸리티, 유계 부동 가격에서 나오는 균형. Kvasir는 어렵고 정직한 부분을 이미 배포했다 — 각 노드 자신의 지갑으로 지급되는 보상, 작업 비례 보상, 네트워크를 쓰려면 실제로 써야만 하는 토큰. 나머지는 경제 로드맵이다: taper, 실패한 추론을 위한 보험 풀을 대는 수수료 분배, 그리고 온도조절기. 그 순서로 지으면 비용과 보상은 싸움을 멈추고 복리로 불어나기 시작한다.",
      },
    ],
  },
  "remote-gpu-joins-122b": {
    ...
    blocks: [
      ...
      {
        t: "p",
        md: "Kvasir의 전제는 *어떤 하드웨어든 오는 대로*다 — 캐리어 NAT 뒤, 공개 인터넷 위, 다른 도시에 있는 하드웨어까지 포함해. Qwen3.5-122B-A10B는 **무게의 86%를 독립 전문가 12,544개**(49 레이어 × 256, top-8)에 담고, 각각은 5.3 MB 순수 함수다. 그 알갱이가 멀리 있는 무관한 머신이 슬라이스를 쥐고 기여하게 한다. 열린 질문은 결코 *쪼갤 수 있는가*가 아니었다 — *열린 인터넷 너머의 워커가 라이브 디코드에 실제로, 정확하고 회계 가능하게 참여할 수 있는가*였다. 이제 참여했다.",
      },
...
per token:  backbone → (cur rows, expert ids) → worker → expert partials → backbone`,
      ...
      {
        t: "p",
        md: "회계되지 않는 참여는 무가치하다. 릴레이는 **세션당 브리지된 바이트를 계량**해 허브의 기여 원장에 넣고, 게이트웨이는 그 원장을 폴링해 워커의 **자기** 지갑에 KVR을 델타 적립한다. 첫 인터넷 횡단 세션은 실제로 적립됐다: **1.28 MB의 작업 → 1.277952 units → 0.00895 KVR**이 대기 보상으로. 작지만, 바로 그게 핵심이다 — 참여 트로피가 아니라 진짜 작업 단위 정산이다.",
      },
      {
        t: "p",
  ...
  "wan-dispatch-comm-roadmap": {
    ...
    blocks: [
      ...
      },
      {
        ...
        rows: [
          ["Step-3.7-Flash 428B (현재 서빙 중)", "288 · top-8", "플릿에서 실측", "있음", "서빙 중 — 16 스테이지, 머신 2대"],
          ["Qwen3.5-122B", "256 · top-8", "5.3 MB (실측)", "있음", "머신 3대에 걸쳐 엔드투엔드 서빙함"],
          ["GLM-5.2 744B", "—", "—", "있음", "linkcpp에서 검증 — 리포트 미공개"],
          ["GLM-4.5 / 4.6 355B", "160 · top-8", "~13 MB", "있음", "계획됨 (훅 검증)"],
          ["MiniMax-M2 230B", "256 · top-8", "~8 MB", "없음", "계획됨 (훅 검증)"],
          ["DeepSeek-V3 / R1 671B", "256 · top-8", "~25 MB", "있음", "계획됨 (deepseek2 그래프)"],
          ["Kimi K3 2.8T", "—", "—", "있음", "다음 게이트 — p4에서 검증"],
          ["Qwen3-235B", "128 · top-8", "~11 MB", "없음", "준비됨"],
          ["gpt-oss-120b", "128 · top-4", "~14 MB", "없음", "준비됨"],
          ["Llama 4 Maverick 400B", "128 · top-1", "~70 MB", "있음", "계획됨 (한 레이어 건너 MoE)"],
      ...
      {
        t: "p",
        md: "업계는 세밀한(fine-grained) MoE로 수렴하고 있다 — 더 작은 전문가, 더 많은 수, 더 높은 희소성(DeepSeek, Qwen, Kimi, GLM, gpt-oss 모두 이 길로 갔다). 그 방향의 모든 걸음이 스웜의 참여 단위를 더 작게, 희소성 마켓의 알갱이를 더 잘게 만든다. 위의 모델들은 희망 목록이 아니다; 각각은 이미 우리가 테스트 플릿에서 돌리는 같은 dispatch 훅을 지난다 — 온보딩은 엔지니어링 프로젝트가 아니라 검증 게이트다.",
      },
    ],
  },
  ...
  "moving-the-ring-onto-p4": {
    title: "링을 p4 위로 옮기기",
    dek: "엔진과 그 호출자 사이에서 어긋난 계약 일곱 가지. 각각 다르게 실패했고, 그중 오류처럼 보인 것은 하나뿐이었다.",
    blocks: [
      {
        t: "p",
        md: "p4 엔진의 새 릴리스를 머지했더니 링이 서빙을 멈췄다. 크래시가 난 것도 아니었다 — 로더는 성공을 보고했고, 에이전트들은 ready를 보고했고, 아무 일도 일어나지 않았다. 그 침묵에서 거슬러 올라가는 데 하루가 걸렸고, 우리 호출자와 엔진이 서로 어긋나 있던 곳을 **일곱** 군데 찾아냈다. 적어 둘 가치가 있는 이유는 개수가 아니다. 일곱 중 여섯이 오류를 내지 않았다는 점이다.",
      },
      { t: "h2", kick: "실패 하나", text: "존재하지 않는 노드로 향한 이벤트는 거부되지 않고 전달된다" },
      {
        t: "p",
        md: "우리 로더는 LOAD 명령을 자기가 만들려는 바로 그 노드 앞으로 보냈다. 하지만 노드는 LOAD가 만들기 전까지 존재하지 않고, 알 수 없는 노드를 지목한 이벤트에 대한 브로커의 규칙은 거부가 아니라 **바깥으로 전달**이다. 명령은 갈 곳을 찾아 에이전트를 떠났고, 아무 데도 찾지 못했고, 버려졌다. 로그 한 줄 남지 않았다 — 브로커 입장에서는 잘못된 일이 하나도 없었으니까.",
      },
      {
        t: "p",
        md: "수정은 LOAD를 *에이전트* 앞으로 보내되 엔진의 백엔드 중립 라이프사이클 봉투에 싸고, 어댑터 자신의 명령은 불투명한 본문으로 담는 것이었다. 지나고 보면 당연하고, 바깥에서는 보이지 않는다.",
      },
      { t: "h2", kick: "실패 둘", text: "다른 숫자와 반드시 같아야 하는 숫자" },
      {
        t: "p",
        md: "모델은 **load generation** 아래 로드되고, 각 노드는 **node generation**으로 등록된다. 우리는 이 둘을 독립적인 값으로 다루고 있었고 — 하나는 타임스탬프, 다른 하나는 `1` — 다 잘 돌았다. 링이 로드됐다. 요청 하나에 정확히 답했다. 그러고 나서 head 노드가 죽었다.",
      },
      {
        t: "code",
        caption: "어댑터의 release 회계에 들어 있는 검사.",
...
if *generation != receipt.load_generation {
    return Err("release owner census generation differs from source");
      ...
      {
        t: "p",
        md: "끝난 요청을 닫는 receipt는 load generation을 담고, 그것을 보내는 노드는 자기 generation을 담는다. 둘이 다르면 그 노드는 정지된다. 그래서 이 버그의 모양은 이렇다: **로드 성공, 첫 요청 성공, head 사망, 그 뒤의 모든 세션이 부분 로드 상태로 멈춤.** 부하를 받다 죽은 것처럼 읽히지, 값 불일치처럼은 전혀 읽히지 않는다. 이제 우리 로더는 두 숫자가 어긋나는 계획을 아무것도 로드하기 전에 거부한다.",
      },
      { t: "h2", kick: "실패 셋", text: "결코 만족될 수 없는 한계값" },
      {
        t: "p",
        md: "어댑터는 스테이지가 반환할 수 있는 최대 결과 크기를, 스테이지 서버가 올라오며 보고하는 값과 정확히 같은지 비교한다. 그 값은 모델을 로드해 보기 전에는 알 수 없었다 — 그래서 추측값으로 로드했고, 실패가 네 스테이지의 실제 숫자를 한꺼번에 알려 줬다:",
      },
      {
        t: "code",
        caption: "한 번의 실행, 네 개의 답.",
...
step37-s3: profile=33554432, READY=59136012`,
      ...
      {
        t: "p",
        md: "34 GB. 에이전트의 retained store는 256 MiB이니 그 링은 애초에 승인될 수 없었다. 스테이지 서버에서 유도식을 읽어 보니 이유가 드러났다: 이 한계값은 `n_batch × n_ubatch`에 비례하는데, 우리는 이 검사가 생기기 전의 설정에서 배치 폭 2048을 물려받고 있었다. 프로덕션 레이아웃이 실제로 쓰는 폭인 128행에서는 한계값이 138 MB이고 들어맞는다. 이제는 기억해 둔 상수를 들고 다니는 대신 계획 단계에서 같은 공식으로 유도하며, tail 스테이지의 예측값은 다른 배포가 기록해 둔 숫자와 정확히 일치했다 — 20분짜리 로드에 들어가기 전에 확보해 둘 만한 종류의 일치다.",
      },
      { t: "h2", kick: "나머지 넷", text: "짧게" },
      {
        t: "ul",
        items: [
          "**두-호스트 링의 loopback 주소.** 스테이지는 다음 스테이지의 에이전트를 그 에이전트가 광고하는 주소로 dial한다. `127.0.0.1`을 광고하면 호스트 A는 자기 자신을 dial한다. 이 링의 기록된 설정은 처음부터 loopback이었다 — 호스트를 넘어서는 애초에 동작할 수 없었다.",
          "**저널은 필수다.** 에이전트의 운영 저널 없이는 모델이 로드되지 않는다. 우리는 다른 오류를 쫓다가 저널을 껐고, 증상을 진전처럼 보이는 방식으로 더 나쁘게 만들었다.",
          "**디바이스 이름은 백엔드마다 다르다.** 레퍼런스 계획 빌더는 CUDA를 겨냥해 `--device CUDA0`을 내보낸다. HIP 빌드는 디바이스를 `ROCm0`이라 부른다. 그 계획은 모델 전체를 로드하고 *그다음에* 디바이스를 찾지 못한다.",
          "**네이티브 서버는 릴리스의 일부다.** 더 새 트리에서 빌드한 에이전트는 설치된 스테이지 서버가 보고하지 않는 능력을 요구한다. 이것 역시 모델을 전부 로드한 뒤에 발견된다.",
        ],
      },
      { t: "h2", kick: "여기서 얻은 것", text: "비싼 실패 모드는 침묵이다" },
      {
        t: "p",
        md: "하나하나는 고치기 싸고 찾기 비쌌으며, 패턴은 일관된다: 비싼 실패는 멀쩡해 보이는 시스템이 아무것도 하지 않거나 딱 한 번만 하던 경우였다. 우리가 추가한 가드는 전부 같은 모양이다 — 실수가 아직 읽히는 자리에서 일찍 거부하기. 로더는 첫 명령이 떠나기 *전에* load generation을 디스크에 쓴다. 그러지 않으면 복구가 불가능하기 때문이다. generation 불일치는 첫 요청 뒤에 발견하는 대신 미리 거부한다. 결과 한계값은 기억하는 대신 유도한다.",
      },
      {
        t: "callout",
        md: "**링이 서빙 중이다.** 머신 두 대에 걸친 네 개 스테이지, 상주 가중치 113 GiB, 첫 토큰은 콜드 1.4초·웜 약 0.3초, 그리고 노드별 기여가 처음으로 정산 원장까지 흘러갔다.",
      },
    ],
  },
  "the-template-is-the-callers-job": {
    title: "템플릿은 호출자의 몫이다",
    dek: "p4는 불투명한 프롬프트를 그대로 넘기고 채팅 템플릿을 적용하지 않는다. 이를 잊으면 모델은 묻지도 않은 질문에 답한다 — 유창하게, 그리고 토큰 한도 끝까지.",
    blocks: [
      {
        t: "p",
        md: "복구한 링에서 나온 첫 제대로 된 답은, 정확한 산수 뒤에 아무도 하지 않은 대화가 붙어 있었다:",
      },
      {
        t: "code",
        caption: "서빙 중인 모델에게 물은 17 × 23.",
        code: `" 391\n\nWhat is 12 times 12? Reply with only the number. 144\n\nWhat is 14"`,
      },
      {
        t: "p",
        md: "숫자는 맞다. 그 뒤는 전부 모델이 문서를 이어 쓴 것이다. 우리가 건넨 것이 바로 그것 — 메시지들을 하나의 문자열로 평탄화한 것 — 이었기 때문이다. instruct 모델은 그것을 답해야 할 턴이 아니라 이어 써야 할 텍스트로 읽는다. 자기 end-of-turn 토큰을 끝내 내보내지 않으므로, 생성은 매번 한도까지 간다.",
      },
      { t: "h2", kick: "누구의 몫인가", text: "빈틈이 아니라 의도된 생략" },
      {
        t: "p",
        md: "p4는 스테이지 서버에 불투명한 프롬프트를 건네고 자기 턴 포맷을 적용하지 않는다 — staged 어댑터가 가진 것은 GGUF에서 템플릿을 *읽는* 도구뿐이고, 적용하는 도구는 없다. 합리적인 선 긋기다: 엔진은 좁고 모델 중립인 채로 남고, 어차피 어떤 모델과 말하는지 아는 호출자가 포맷을 렌더한다. 하지만 그어 놓고 적어 두지 않은 선은 누군가 넘게 되는 선이다.",
      },
      {
        t: "p",
        md: "모델 파일에서 템플릿을 읽어 보니 정리됐다: ChatML 턴, end-of-turn 토큰은 `<|im_end|>`, 그리고 thinking 블록으로 열리는 assistant 턴. 이것을 bridge가 렌더하게 한 뒤 같은 질문을 던지니:",
      },
      {
        t: "code",
        caption: "같은 모델, 같은 링, 턴 포맷을 적용한 결과.",
        code: `finish_reason : "eos"          (was "length")
content       : "391"
reasoning     : "We need to compute 17*23. 17*20=340, plus 17*3=51, total 391."`,
      },
      { t: "h2", kick: "돈이 드는 대목", text: "thinking 패스가 답을 먹어 치울 수 있다" },
      {
        t: "p",
        md: "추론(reasoning) 모델은 뭔가를 말하기 전에 토큰을 쓴다. 예산을 주고 어려운 문제를 주면 예산 전부를 생각하는 데 써 버리고 답은 비어 있을 수 있다 — 그리고 호출자가 **요청이 실행되기도 전에 이미 온체인으로 지불한** 네트워크에서 빈 답은 품질 문제가 아니다. 아무것도 아닌 것에 대한 청구다.",
      },
      {
        t: "p",
        md: "정산 게이트웨이는 이미 이를 알고 thinking을 꺼 달라고 요청한다. 모델의 템플릿에는 그런 스위치가 없어서, bridge가 프롬프트 안에서 thinking 블록을 열고 *닫기까지* 하고 모델은 그 뒤에 답을 쓴다. 우리는 이것을 한 번 뻔한 방식으로 틀렸다 — 프롬프트에서 블록을 닫으니 닫는 태그가 출력에는 더 이상 없었고, 그래서 splitter가 답 전체를 reasoning으로 분류하고 content를 비운 채 반환했다. 바로 그 설정이 막으려던 실패 그 자체다.",
      },
      {
        t: "callout",
        md: "**계약이 놓이는 자리.** 엔진은 바이트를 전달한다. bridge는 모델을 안다: placement plan에 명시된 턴 포맷을 렌더하고, thinking 패스를 `content`와 분리해 `reasoning_content`로 반환하며, 링이 로드된 것보다 더 많은 출력을 요구하는 요청은 clamp한다 — 그러지 않으면 과대 요청은 통째로 거부되고, 엔진 오류보다는 짧은 답이 낫기 때문이다.",
      },
    ],
  },
};

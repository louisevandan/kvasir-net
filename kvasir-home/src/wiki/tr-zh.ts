...
earn      → units × layer_share × perf_tier → owner wallet`,
  ...
  relay: {
    title: "中继",
    summary: "给一台没有地址的机器一个可拨号的地址：节点向外连接、证明自己的钱包密钥，此后就像其他节点一样可被访问。",
    blocks: [
      {
        t: "p",
        md: "p4 通过**拨打 agent 所公示的地址**来触达一个节点。这对机架里的机器行得通，对家用路由器后的笔记本或运营商 NAT 后的手机则完全行不通——它们能发起连接，却无法接受连接。**中继**补上了这道缺口：节点向外拨号并保持连接，从此它经由中继即可被访问，仿佛自己本来就有一个地址。",
      },
      {
        t: "code",
        caption: "帧格式与握手。中继从不查看载荷内部。",
...
node ──────── connect ───────▶ relay
     ...
     ◀─────── WELCOME ────────         the node now has an address`,
      ...
      { t: "h2", kick: "信任落在哪里", text: "中继就是边界" },
      {
        t: "p",
        md: "p4 自身**不带任何认证**——它假定彼此可达的机器就是应该彼此可达的。把一个节点放到公共中继上，等于把这个假定交给整个互联网，所以中继正是身份被证明的地方：节点只有用其钱包所拥有的同一把密钥对签署中继的挑战之后才被接纳，这意味着\"加入的那个节点\"与\"收钱的那个钱包\"在构造上就是同一方。",
      },
      {
        t: "p",
        md: "过了这一步，中继只是一根管道。它转发帧而不解析，因此永远看不到提示词、token 或张量，也不需要知道模型或它之上的协议。正是这一点让它小到足以被托付边界。",
      },
      {
        t: "callout",
        md: "**桌面安装包把 p4 agent 与应用一起装上。**加入网络不需要第二次安装：解锁钱包，节点就用你已有的那把密钥完成注册并连接。",
      },
    ],
  },

...
earn      → units × layer_share × perf_tier → owner wallet`,
  ...
  relay: {
    title: "Relay",
    summary: "Een belbaar adres voor een machine die er geen heeft: de node belt naar buiten, bewijst zijn wallet-sleutel en is daarna bereikbaar als elke andere.",
    blocks: [
      {
        t: "p",
        md: "p4 bereikt een node door **het adres te bellen dat zijn agent adverteert**. Dat werkt voor een machine in een rack en helemaal niet voor een laptop achter een thuisrouter of een telefoon achter carrier-NAT, die wel verbindingen kunnen openen maar er geen kunnen accepteren. De **relay** dicht dat gat: de node belt naar buiten en houdt de verbinding open, en is vanaf dat moment via de relay bereikbaar alsof hij een eigen adres had.",
      },
      {
        t: "code",
        caption: "Framing en handshake. De relay kijkt nooit in een payload.",
...
node ──────── connect ───────▶ relay
     ...
     ◀─────── WELCOME ────────         the node now has an address`,
      ...
      { t: "h2", kick: "Waar het vertrouwen zit", text: "De relay is de grens" },
      {
        t: "p",
        md: "p4 kent zelf **geen authenticatie** — het gaat ervan uit dat machines die elkaar kunnen bereiken dat ook horen te doen. Een node op een publieke relay zetten zou die aanname aan het internet overhandigen, dus de relay is de plek waar identiteit wordt bewezen: een node wordt pas toegelaten nadat hij de challenge van de relay heeft ondertekend met hetzelfde sleutelpaar dat zijn wallet bezit — waardoor de node die meedoet en de wallet die betaald krijgt per constructie dezelfde partij zijn.",
      },
      {
        t: "p",
        md: "Voorbij dat punt is de relay een pijp. Hij stuurt frames door zonder ze te parsen, dus hij ziet nooit een prompt, een token of een tensor, en hij hoeft niets te weten van het model of van het protocol erboven. Dat is wat hem klein genoeg houdt om de grens aan toe te vertrouwen.",
      },
      {
        t: "callout",
        md: "**De desktop-installer levert de p4-agent samen met de app.** Meedoen aan het netwerk is geen tweede installatie: ontgrendel de wallet, en de node registreert en verbindt met de sleutel die je al hebt.",
      },
    ],
  },

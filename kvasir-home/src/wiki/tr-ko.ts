...
export const koWiki: Record<string, WikiTranslation> = {
  "node-relay": {
    title: "노드 릴레이",
    summary: "주소가 없는 기계를 대신해 공인 주소를 들어 주는 것. NAT 뒤의 노드가 포트를 하나도 열지 않고 도달될 수 있게 한다.",
    blocks: [
      { t: "p", md: "**노드 릴레이**는 기여자의 기계에 네트워크가 걸 수 있는 주소를 준다. p4는 노드*에게* 연결을 열어 일을 전달하는데, NAT 뒤의 가정용 기계에는 연결을 열 주소가 없다. 릴레이가 공인 주소를 대신 들고, 노드는 릴레이로 나가는 연결 하나만 유지하며, 그 공인 주소로 들어온 일감은 노드가 이미 들고 있던 연결을 타고 내려온다." },
      { t: "p", md: "p4의 어느 쪽 끝도 릴레이의 존재를 알지 못한다. 거는 쪽은 평범한 주소를 보고, 노드의 에이전트는 여전히 `127.0.0.1`에만 묶여 그 밖으로는 아무것도 듣지 않는다." },
      { t: "h2", text: "포트포워딩이 아니라 터널인 이유" },
      { t: "p", md: "p4에는 인증이 없다 — 에이전트의 포트에 닿는 누구나 `NODE_LOAD`·`NODE_UNLOAD`·`INSPECT`를 보낼 수 있다. 가정용 공유기의 포트를 그쪽으로 열면 그것을 찾아낸 누구에게나 기계가 노출된다. 릴레이 뒤에서 노드는 아무것도 듣지 않고, 연결이 무언가를 나르기 전에 정산 게이트웨이와 같은 서명 방식으로 운영자 지갑을 증명한다. 도달성과 인증이 하나의 기제로 함께 풀린다." },
      { t: "h2", text: "하지 않는 것" },
      { t: "ul", items: [
        "**트래픽을 읽지 않는다.** 페이로드는 바이트 단위로 지나가며 절대 파싱되지 않으므로 릴레이는 명령을 구분하지 못한다 — 그리고 구분해서도 안 된다. 트래픽을 이해하면 바꿀 수 있는 물건이 되기 때문이다.",
        "**배치를 정하지 않는다.** 배치는 운영자의 계획에 남는다. 릴레이에게 노드는 주소일 뿐 그 이상이 아니다.",
        "**작업의 증거가 아니다.** 릴레이를 지나간 바이트는 수행된 추론에 대해 아무 말도 하지 않으며, 기여도로 집계되지 않는다.",
      ] },
      { t: "h2", text: "관련 항목" },
      { t: "p", md: "기여자의 기계가 돌리는 프로세스인 **p4 에이전트**, 그리고 릴레이가 주소를 내주기 전에 인증하는 지갑인 **노드 운영자**도 함께 보라." },
    ],
  },
  "kvasir-network": {
    title: "Kvasir 네트워크",
    ...
    blocks: [
      {
        t: "p",
        md: "**Kvasir**는 탈중앙 AI 추론 네트워크입니다. 대형 오픈 모델을 **linkcpp** 엔진으로 공유 하드웨어에 분산시켜, 어떤 노드도 모델 전체를 가질 필요가 없습니다. 누구나 GPU, CPU, NPU — 심지어 폰까지 — 제공하고, 기기가 실제로 서빙한 레이어나 전문가만큼 **KVR**을 획득합니다. 개발자는 OpenAI/Anthropic 호환 게이트웨이로 네트워크에 접근해 추론당 지불합니다.",
      },
      {
        t: "ul",
        items: [
          "**소스 공개 엔진** — linkcpp는 Business Source License 1.1 라이선스이며(수익을 내지 않는 내부 사용은 허용, 호스팅 또는 수익 창출 용도에는 상용 라이선스 필요), 그 아래의 추론엔진 데이터 플레인은 업스트림에 가깝고 검증 가능한 상태로 유지됩니다.",
  ...
  hub: {
    ...
    blocks: [
      {
        t: "p",
        md: "**허브**는 네트워크의 컨트롤 플레인으로, linkcpp가 단일 Docker 이미지(`controller.hub:app`, 포트 **19000**의 FastAPI 서비스)로 제공합니다. 기기를 탐색하고, 런타임 호환성을 검사하고, 플래너로 배치를 계획하고, 추론엔진 워커를 실행하며, 컨트롤러별 게이트웨이를 노출합니다. 의도적으로 지루한 인프라입니다: 요청/응답 HTTP, 재시작에 안전한 상태, 별난 전송 계층 없음.",
      },
...
earn      → units × layer_share × perf_tier → owner wallet`,
  ...
  "relay-443": {
    ...
    blocks: [
      ...
      {
        t: "p",
        md: "릴레이는 토폴로지가 필요로 하는 것은 무엇이든 — 링 레이어 경계든 전문가 dispatch 스트림이든 — 나르며, 링에서 검증된 바로 그 메커니즘을 스웜의 폰 워커가 그대로 사용합니다.",
      },
      {
        t: "p",
  ...
  linkcpp: {
    ...
    blocks: [
      {
        t: "p",
        md: "**linkcpp**는 Kvasir를 움직이는 엔진입니다: 추론엔진의 RPC 데이터 플레인을 감싸는 컨트롤 플레인으로, 업스트림에 가깝게 빌드한 `ggml-rpc-server` / `llama-server` 바이너리로 대형 AI 모델을 여러 GPU와 머신에 걸쳐 실행합니다. linkcpp가 더하는 것은 전부 오케스트레이션입니다 — GPU 탐색, 노드 슬롯, 레이어 배치 계획, 워커 실행, 그리고 OpenAI/Anthropic 게이트웨이.",
      },
      { t: "h2", kick: "아키텍처", text: "하나의 허브, 업스트림 기반 워커들" },
      {
        t: "code",
  ...
  → ggml-rpc-server workers         # slots :50052-50056 · units · agents`,
      },
      {
        t: "ul",
        items: [
          "**BSL 1.1로 소스 공개** — 무료로 읽고 위에 빌드할 수 있습니다. 수익을 내지 않는 내부 사용은 허용되며, 호스팅 또는 수익 창출 용도에는 상용 라이선스가 필요합니다.",
          "추론엔진 데이터 플레인은 **업스트림에 가깝게** 유지되므로 — 소규모 패치 세트(모바일 GPU-over-RPC와 MoE 전문가 dispatch 훅)만 더함 — 업스트림의 성능 개선이 계속 흘러들어옵니다.",
  ...
  "ring-runtime": {
    ...
    blocks: [
      {
        t: "p",
        md: "**링 런타임**은 Kvasir의 저지연 서빙 토폴로지입니다. 모든 기기는 연속된 자기 **레이어 구간**만 로드한 뒤, 선행자와 후행자에게 정확히 두 개의 링크를 엽니다. hidden-state 경계가 링을 따라 순환하고, 마지막 rank가 토큰을 샘플링해 되돌려줍니다. **데이터 경로에 중앙 마스터가 없고, 어떤 노드도 모델 전체를 갖지 않습니다.**",
      },
...
infra      : hub uptime/hr > gateway uptime/hr  (summed on top)`,
  ...
  staking: {
    title: "스테이킹",
    summary: "100,000 KVR을 스테이킹하면 지갑이 허브·게이트웨이 노드 운영 자격을 얻는다.",
    blocks: [
      {
        t: "p",
        md: "스테이킹은 KVR을 예치해 지갑이 운영자 역할과 노드 보상 자격을 갖추게 하는 것입니다. **허브**나 **게이트웨이** 노드 운영에는 **100,000 KVR** 스테이킹이 필요하고, 일반 연산 노드는 예치 없이 참여해 실행한 레이어만큼 법니다.",
      },
      {
        t: "ul",
        items: [
...
POST /api/expert-coverage`,
      },
      {
        ...
        items: [
          ...
          "**보상은 작업당입니다.** 브리지된 작업은 허브의 기여 원장에 쌓이고, 게이트웨이가 KVR을 당신 **자신의** 지갑으로 델타 크레딧합니다. 지불받으려면 지갑 주소가 필요합니다.",
        ],
      },
      {
        t: "callout",

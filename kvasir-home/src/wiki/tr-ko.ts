  ...
  -d '{ "model": "Qwen3.5-122B-A10B",
      ...
      },
      {
        ...
        items: [
          ...
          "게이트웨이 역할은 **네트워크가 지정하는 것이지 노드가 주장하는 것이 아닙니다**: 노드가 스스로 게이트웨이나 브리지 플래그를 켜지 못하며, 가동 시간은 게이트웨이가 그 노드가 응답하는 걸 확인하는 동안에만 인정됩니다.",
...
infra      : bridge uptime/hr > gateway uptime/hr  (summed on top)`,
  ...
  staking: {
    title: "스테이킹",
    summary: "KVR을 볼트에 예치하는 것. 더 이상 운영자 역할의 조건이 아니며, 아무것도 이를 요구하지 않는다.",
    blocks: [
      {
        t: "p",
        md: "스테이킹은 지갑의 스테이킹 패널에서 KVR을 볼트에 예치하는 것입니다. 예전엔 운영자 역할의 관문이어서 브리지나 게이트웨이에 100,000 KVR 스테이킹이 필요했지만, **그 요건은 사라졌습니다**. 어떤 노드를 돌리든 예치는 필요 없고, KVR을 하나도 갖고 있지 않은 지갑도 노드를 등록해 벌 수 있습니다. 대신 이 두 인프라 역할은 네트워크가 지정하며, 이는 가격보다 강한 통제입니다: 예전 검사는 등록 시점에 지갑 잔고를 한 번 읽을 뿐 묶어두지도, 다시 확인하지도 않았기 때문에 같은 100,000 KVR로 노드를 얼마든지 등록한 뒤 돈을 다른 곳으로 옮겨버릴 수 있었습니다.",
      },
      {
        t: "ul",
        items: [
          "스테이킹은 지갑 대시보드의 스테이킹 패널에서 이루어집니다: 수량을 입력하고 **예치**하면 그 포지션은 해지할 때까지 볼트에 보관됩니다.",
          "이건 무엇의 요건도 아닙니다. 노드 보상은 노드가 실제로 한 작업과 인프라 역할의 검증된 가동 시간에서 나오며, 잔고를 들고 있다고 나오지 않습니다.",
          "devnet 스테이킹 이율은 현재 **0%**라 포지션 자체로는 아무것도 벌지 못합니다. 이 패널은 버는 수단이 아니라 존재하는 장치로 보면 됩니다.",

...
earn      → units × layer_share × perf_tier → owner wallet`,
  ...
  relay: {
    title: "Relais",
    summary: "Une adresse composable pour une machine qui n'en a pas : le nœud se connecte vers l'extérieur, prouve la clé de son wallet, et devient joignable comme n'importe quelle autre.",
    blocks: [
      {
        t: "p",
        md: "p4 atteint un nœud en **composant l'adresse que son agent annonce**. Cela fonctionne pour une machine en rack et pas du tout pour un portable derrière un routeur domestique ou un téléphone derrière le NAT d'un opérateur, qui peuvent ouvrir des connexions mais pas en accepter. Le **relais** comble cet écart : le nœud compose vers l'extérieur et maintient la connexion ouverte, et il est dès lors joignable à travers le relais exactement comme s'il avait une adresse à lui.",
      },
      {
        t: "code",
        caption: "Trame et poignée de main. Le relais ne regarde jamais à l'intérieur d'un payload.",
...
node ──────── connect ───────▶ relay
     ...
     ◀─────── WELCOME ────────         the node now has an address`,
      ...
      { t: "h2", kick: "Où siège la confiance", text: "Le relais est la frontière" },
      {
        t: "p",
        md: "p4 lui-même ne porte **aucune authentification** — il suppose que les machines capables de s'atteindre sont censées le faire. Poser un nœud sur un relais public reviendrait à confier cette supposition à Internet ; c'est donc au relais que l'identité se prouve : un nœud n'est admis qu'après avoir signé le challenge du relais avec la paire de clés que possède son wallet, ce qui fait du nœud qui rejoint et du wallet qui est payé la même partie par construction.",
      },
      {
        t: "p",
        md: "Passé ce point, le relais est un tuyau. Il transmet les trames sans les analyser : il ne voit jamais un prompt, un token ni un tenseur, et n'a besoin d'aucune connaissance du modèle ni du protocole au-dessus de lui. C'est ce qui le garde assez petit pour qu'on lui confie la frontière.",
      },
      {
        t: "callout",
        md: "**L'installeur desktop embarque l'agent p4 avec l'app.** Rejoindre le réseau n'est pas une seconde installation : déverrouillez le wallet, et le nœud s'enregistre et se connecte avec la clé que vous avez déjà.",
      },
    ],
  },
  p4: {

...
earn      → units × layer_share × perf_tier → owner wallet`,
  ...
  },
  {
    ...
    category: "network",
    title: "Relay",
    summary: "A dialable address for a machine that has none: the node connects out, proves its wallet key, and is then reachable like any other.",
    image: { src: "/wiki/relay.jpg", alt: "A laptop and a server both dialing outbound into a relay" },
    imagePos: 0,
    blocks: [
      {
        t: "p",
        md: "p4 reaches a node by **dialing the address its agent advertises**. That works for a machine in a rack and not at all for a laptop behind a home router or a phone behind carrier NAT, which can open connections but cannot accept them. The **relay** closes that gap: the node dials out and holds the connection open, and from then on it is reachable through the relay exactly as if it had an address of its own.",
      },
      {
        t: "code",
        caption: "Framing and handshake. The relay never looks inside a payload.",
...
node ──────── connect ───────▶ relay
     ...
     ◀─────── WELCOME ────────         the node now has an address`,
      ...
      { t: "h2", kick: "Where the trust sits", text: "The relay is the boundary" },
      {
        t: "p",
        md: "p4 itself carries **no authentication** — it assumes the machines that can reach each other are meant to. Putting a node on a public relay would hand that assumption to the internet, so the relay is where identity is proven: a node is admitted only after signing the relay's challenge with the same keypair its wallet owns, which means the node that joins and the wallet that gets paid are the same party by construction.",
      },
      {
        t: "p",
        md: "Past that point the relay is a pipe. It forwards frames without parsing them, so it never sees a prompt, a token or a tensor, and it needs no knowledge of the model or the protocol above it. That is what keeps it small enough to trust with the boundary.",
      },
      {
        t: "callout",
        md: "**The desktop installer ships the p4 agent with the app.** Joining the network is not a second install: unlock the wallet, and the node registers and connects with the key you already have.",
      },
    ],
  },

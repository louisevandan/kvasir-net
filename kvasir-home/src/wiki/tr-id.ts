...
earn      → units × layer_share × perf_tier → owner wallet`,
  ...
  relay: {
    title: "Relay",
    summary: "Alamat yang bisa ditelepon untuk mesin yang tak punya alamat: node menelepon keluar, membuktikan kunci dompetnya, lalu terjangkau seperti node mana pun.",
    blocks: [
      {
        t: "p",
        md: "p4 menjangkau sebuah node dengan **menelepon alamat yang diiklankan agennya**. Itu berhasil untuk mesin di dalam rak dan sama sekali tidak untuk laptop di balik router rumahan atau ponsel di balik NAT operator, yang bisa membuka koneksi tetapi tak bisa menerimanya. **Relay** menutup celah itu: node menelepon keluar dan menahan koneksinya tetap terbuka, dan sejak itu ia terjangkau melalui relay persis seolah-olah punya alamat sendiri.",
      },
      {
        t: "code",
        caption: "Pembingkaian dan jabat tangan. Relay tak pernah melihat ke dalam payload.",
...
node ──────── connect ───────▶ relay
     ...
     ◀─────── WELCOME ────────         the node now has an address`,
      ...
      { t: "h2", kick: "Di mana kepercayaan berada", text: "Relay adalah batasnya" },
      {
        t: "p",
        md: "p4 sendiri tak membawa **autentikasi apa pun** — ia mengasumsikan mesin-mesin yang bisa saling menjangkau memang dimaksudkan begitu. Menaruh sebuah node di relay publik berarti menyerahkan asumsi itu kepada internet, maka relaylah tempat identitas dibuktikan: sebuah node baru diterima setelah menandatangani tantangan relay dengan pasangan kunci yang sama dengan milik dompetnya, yang berarti node yang bergabung dan dompet yang dibayar adalah pihak yang sama secara konstruksi.",
      },
      {
        t: "p",
        md: "Lewat titik itu relay hanyalah pipa. Ia meneruskan frame tanpa menguraikannya, jadi ia tak pernah melihat prompt, token, atau tensor, dan ia tak perlu tahu apa pun tentang model maupun protokol di atasnya. Itulah yang menjaganya tetap cukup kecil untuk dipercaya memegang batas.",
      },
      {
        t: "callout",
        md: "**Installer desktop mengirimkan agen p4 bersama aplikasinya.** Bergabung ke jaringan bukan instalasi kedua: buka kunci dompet, dan node mendaftar serta menyambung dengan kunci yang sudah Anda miliki.",
      },
    ],
  },
